<!-- MIRRORED from `docs/AUTOMATION-API.md` in the aowlspt repo by tools/gendocs.py. -->
<!-- DO NOT EDIT HERE. Edit the source and re-run the generator. -->

::: info Mirrored document
This page is copied verbatim from `docs/AUTOMATION-API.md` in the aowlspt repository and regenerated by `tools/gendocs.py`. Edits made here are lost on the next run.
:::
