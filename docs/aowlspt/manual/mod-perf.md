---
title: "What a mod may do on the load path"
---

# MOD-PERF — what a mod may do on the load path, and the number that proves each rule

::: info From the aowlspt repository — [`docs/MOD-PERF.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/MOD-PERF.md)
...
**The budget for what a mod may do while the game is loading, with the measurement behind each rule.**
...
timestamp window or with [`tools/bigjson.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/bigjson.py) / a `json.load` of `db.json`,
...
| countdown screen     | 13:59:26.0   | 150 s   | 11.1 s  | <code>EFT.&lt;ShowCountdown&gt;</code> first log at 13:59:26.041 |
...
| | vanilla `db.json` `locations.woods.base` | served by [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) (flag OFF) |
|---|---|---|
...
Source: [`mods/tarkov/emu/raid.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/raid.nim) `offlineScavWavesWith` (waves),
...
awaits the `Task.WhenAll` of the <code>Time &lt; 0</code> waves before `Run(2)` (disassembly
...
* <code>BotStart для неволнового спавна &lt;= 0</code> ("BotStart for non-wave spawn &lt;= 0")
...
* <code>spawn process error &#95;inSpawnProcess:N. &gt; &#95;maxBots:0</code> on every spawn — the
...
### 1.4 The fix: `stagedStart` (default OFF) in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)

Settings (F12 → Bots/Spawns; [`mods/tarkov/config.json`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/config.json)):
...
logs one line <code>staged-start readback &lt;Id&gt;: ON&#124;OFF -- N wave(s) at Time:-1 (B bots), M deferred to A..B s (C bots), K trickle wave(s) ...; BotStart x -&gt; y; bosses moved to Time T: n</code>, and the boss roll line now names the clock:
...
INFERRED, not yet measured: that a `boss*` entry with <code>Time &gt;= 0</code> is delayed
...
| manager | `mgr/refresh.nim:900 refreshTick` | <code>926 if gEnabled and gUrl.len &gt; 0 and not gPinnedPath:</code> | one comparison when idle |
...
<code>the backend wants aowl.&lt;mod&gt; loaded; queueing</code> **4.766 s** ×6, every mod's
...
1. **Serve at raid start only what the client needs to open the raid.** Our
   ...
   Pattern: put the vanilla count on the <code>Time &lt; 0</code> arm and the rest on the
...
2. **Never write a value the client rejects and then keep going.**
   `BotStart 0` → <code>BotStart для неволнового спавна &lt;= 0</code> and the vanilla
...
* **The host log is one file per install, overwritten per run.** The 13:56
  ...
  numbers above are from a later run of the same build. [`tools/hostlog.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostlog.py)
  has no `--session`. A per-run copy under <code>D:&#92;Aowlspt&#92;Logs&#92;&lt;ts&gt;&#92;</code> next to
...
* **No offline runner for a mod's self-check.** `selfCheckRaid` runs only
  ...
  [`tools/emutest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/emutest.nim)), so a subagent that may not deploy cannot prove its own
