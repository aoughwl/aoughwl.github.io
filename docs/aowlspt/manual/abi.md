...
::: info From the aowlspt repository — [`docs/ABI.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/ABI.md)
...
[`abi/aowlspt_abi.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_abi.h) is the whole contract. This document explains the decisions
...
barrier the optimiser cannot see through. [`tests/abi_layout.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/abi_layout.c) pins its size and
...
[`tests/detour_test.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_test.c) — and it carries its own `size` like everything else here.
...
lives in [`abi/aowlspt_frame.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_frame.h), not in `aowlspt_abi.h`, which declares only the
`const AowlPatchFrame*` a typed handler is given.)

Revision 3 also fixed a way of *reading* `size` that was right exactly once. A
mod tested a capability with <code>size &gt;= sizeof(AowlHostApi)</code>, which is true while
...
<code>size &gt;= sizeof(HostApi)</code>, so appending `patch_typed` would have made every
revision-3 host answer "no addresses" to a mod that only wanted addresses. It
tests `HostApiSizeRev3` now. [`tests/abi_layout.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/abi_layout.c) pins them to
...
`aowl_hostapi_arm_live` (in [`abi/aowlspt_live.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_live.h), which only it includes), the
backend with `aowl_hostapi_arm_notify` ([`abi/aowlspt_notify.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_notify.h)), the simulator
...
[`abi/aowlspt_notify.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_notify.h), which only the backend includes) installs refusing
...
[`tests/abi_layout.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/abi_layout.c) computes these with the real compiler over the real header;
...
`exportMod` in [`aowl/src/aowlspt.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt.nim) writes all three for you.

## The escape hatch

<code>call(target, args) -&gt; result</code> invokes a host member by name:
...
that goes is the host's problem. [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) builds the JSON the client
dispatches on and calls this, [`backend/websocket.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/websocket.nim) finds the connection and
frames it, and [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h) owns the socket, the read loop and the write
...
[`mods/tarkov/emu/notify.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/notify.nim), a loop over the bound sessions — and would be a
...
become 23.3, 24.9 and 37.3 ns. [`tools/perfbench.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/perfbench.nim) prints the table and
`aowl test` runs it every time; [PERF.md](./perf) has the whole of it,
...
rather than asserted: [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) fires a typed postfix twenty thousand
...
runs per entity per frame is a frame tax rather than a feature. [`mods/fov`](https://github.com/aoughwl/aowlspt/tree/main/mods/fov) and
...
whether it got it. See [IL2CPP.md](./il2cpp).
...
[`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) — the loader all three hosts share — can take one
...
That is what live enable/disable is built on: [`host/common/modcontrol.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modcontrol.nim)
answers [`mods/manager`](https://github.com/aoughwl/aowlspt/tree/main/mods/manager) over the event channel, and every host drains its queue
from its own loop rather than unloading inside the emit.
[MODMANAGER.md](./modmanager) is the whole of it, including what is refused and
...
Precisely, as of commit `be5598a` and in [`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim): `tickMods`
