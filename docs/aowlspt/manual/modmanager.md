---
title: "Changing the mod set while everything runs"
---

# Changing the mod set while the server and the game are running

::: info From the aowlspt repository — [`docs/MODMANAGER.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/MODMANAGER.md)
...
|---|---|---|
| `aowlspt-backend` (server) | **yes**, fully | `aowl test` → "Live mod control", [`tools/livectl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/livectl.nim) |
| `aowlspt-host-il2cpp` (client) | **the mechanism is wired end to end, and it reports back**, but has never been run against BSG's client | the mock runtime under [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp); the wire itself by [`tests/modreport`](https://github.com/aoughwl/aowlspt/tree/main/tests/modreport) (the host half) and [`tests/modclient`](https://github.com/aoughwl/aowlspt/tree/main/tests/modclient) (the manager half) |
| `aowlspt-sim` | **yes** — it loads one mod, and `modcontrol` is wired the same way it is on the backend | <code>aowl run &lt;mod&gt;</code>, and `--emit aowlspt.host.mods.list` |
...
The manager ([`mods/manager`](https://github.com/aoughwl/aowlspt/tree/main/mods/manager), guid `aowl.manager`) is an ordinary backend mod. It
reads [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json), resolves it against your selection, and serves the
result:
...
[`host/common/modcontrol.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modcontrol.nim) is the host half, and it is shared by both hosts —
the wire is the same on either side and two implementations of it would drift.
[`mods/manager/mgr/control.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/control.nim) is the mod half. The *loader* underneath both is
shared now too: [`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) owns discovery, the version probe,
...
* [`backend/aowlbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/aowlbackend.nim) — `modcontrol.drain()` in the serve loop, once a
  tick.
* [`host/Aowlspt.Host.Il2Cpp/aowlhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/aowlhost.nim) — `modcontrol.drain()` in the tick
...
detour the game still jumps to. See "Unloading" in [ABI.md](./abi).
...
is still exactly `proc modEnter*`. Note that [`backend/aowlbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/aowlbackend.nim) is under
...
[ARCHITECTURE.md](./architecture) quotes 11.2 ns for the shipped bodies and
...
[`backend/modrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/modrace.nim) is the reproducer, and **`aowl test` runs it.** This
paragraph used to say it was "the one gate here that `aowl test` does not run:
nothing in [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) mentions it and no `modrace.exe` is built by the
gate". That stopped being true: `tools/aowl.nim:1297-1314` compiles
[`backend/racemod.c`](https://github.com/aoughwl/aowlspt/blob/main/backend/racemod.c), builds `modrace.exe` and runs it as
...
which is the number [ARCHITECTURE.md](./architecture) quotes.

## The client half, as the source has it

Read [`host/Aowlspt.Host.Il2Cpp/aowlhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/aowlhost.nim) if this section and the code ever
...
| `r` | records joined by `!`, each <code>&lt;guid&gt;&#126;&lt;want&gt;&lt;outcome&gt;&#91;&#126;&lt;code&gt;&#93;</code> |
| `more=1` | the rows did not all fit in the path; the rest come on later polls |

<code>&lt;want&gt;</code> is `+` for "should be loaded" and `-` for "should be unloaded".
<code>&lt;outcome&gt;</code> is one letter from a closed set:
...
<code>&lt;code&gt;</code> appears only on `s`, `r` and `d`, and is one of `noartifact`,
...
received. [`mods/manager/mgr/clientreport.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/clientreport.nim) is shaped around making that hard
...
<code>/aowlspt/mods/client/&lt;ver&gt;</code> poll lands and `applyDesired` unloads what should
...
needs a change in [`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) as well as in the manager, which is
...
[`tools/livectl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/livectl.nim), run by `aowl test` under the heading **Live mod control**.
It stages a root with the manager, the Tarkov emulator and
[`examples/gameserver`](https://github.com/aoughwl/aowlspt/tree/main/examples/gameserver), writes a three-mod registry of its own, starts the
...
* a reloaded mod is a fresh instance: [`examples/gameserver`](https://github.com/aoughwl/aowlspt/tree/main/examples/gameserver) counts the requests
...
the game. [`tests/modreport`](https://github.com/aoughwl/aowlspt/tree/main/tests/modreport) drives `modcontrol`'s ledger directly — no host, no
...
the backend has stopped naming). [`tests/modclient`](https://github.com/aoughwl/aowlspt/tree/main/tests/modclient) drives the other end,
[`mods/manager/mgr/clientreport.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/clientreport.nim), exactly **90** checks (both counts re-run
...
manager against polls issued by hand with the report on them. [`tools/hostharness.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostharness.nim)
runs the IL2CPP host against [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp), which is what exercises resolve,
...
with <code>--manager &lt;url&gt;</code> drives all thirteen modes against a running manager and
...
[`registry/fakeregistry.py`](https://github.com/aoughwl/aowlspt/blob/main/registry/fakeregistry.py), which has thirteen entries. *44* is only **partly**
...
gate execution on). [`registry/README.md`](https://github.com/aoughwl/aowlspt/blob/main/registry/README.md) states the shape it would take if that
...
them apart from a published one. A refresh reports what moved — <code>+ &#91;added&#93; - &#91;removed&#93; &#126; &#91;name 0.1.0 -&gt; 9.9.9&#93;</code> — and names any of your selections that now
...
* **Nothing here has ever run against BSG's client.** The IL2CPP host has been
  ...
  process with no runtime in it at all. [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp) sets out what that
