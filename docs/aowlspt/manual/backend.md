...
::: info From the aowlspt repository — [`docs/BACKEND.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/BACKEND.md)
...
and prints it; [PERF-SERVER.md](./perf-server) is what it is for.
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov), documented in [EMULATOR.md](./emulator). It imports `aowlspt`
...
[`examples/gameserver`](https://github.com/aoughwl/aowlspt/tree/main/examples/gameserver) is that, complete and tested.
...
(<code>EFT.&lt;DownloadTexture2D&gt;d&#95;&#95;218::MoveNext</code>, RVA `0xA7FF70`; the request is built
...
of a walk from the document root — see [PERF-SERVER.md](./perf-server).
...
rebuild are the last section of [PERF-SERVER.md](./perf-server).
...
- **A poller and a pool of request workers.** One thread owns the listening
  ...
  connections and 64 MiB of buffered request, in [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h), which
...
- **A websocket, for the notifier.** A request carrying `Upgrade: websocket` is
  ...
  so in [`mods/tarkov/emu/notify.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/notify.nim). The poll is still there and still answers
  every client that does not upgrade.

  The split is stated where it falls. [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h) owns frame
  *boundaries* and the read-path refusals — an unmasked client frame, a length
  ...
  [`backend/websocket.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/websocket.nim), because it is `Sec-WebSocket-Key` + the well-known
  GUID + SHA-1 + base64 and nimony ships both; so is every byte that goes out,
  which is the rule the rest of the server already keeps. [`tools/wstest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/wstest.nim) is
  ...
  **The poller never waits on the websocket write lock.** There is one lock for
  ...
  lock. [`tests/ws_stall.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/ws_stall.c) is the reproducer — it holds the lock with real
  ...
  edit to [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim)); build and run it by hand:
...
- Three receive deadlines, not one timeout: 5 s idle between requests on a
  ...
  stalled connection wakes nobody at all. [PERF-SERVER.md](./perf-server) has
  the measurement.
...
- The static tables are served out of a compressed-body cache keyed on the
  ...
  scan. The numbers and the reasoning are in [PERF-SERVER.md](./perf-server).
- **What is shared between the threads, and what guards it.** Sixteen workers
  ...
  [`backend/dbrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/dbrace.nim) is the gate for the first row: readers and writers on
  ...
  [`backend/modrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/modrace.nim) is the proof, and like `dbrace` it asserts on answers:
  ...
  `racemod.dll` (built from [`backend/racemod.c`](https://github.com/aoughwl/aowlspt/blob/main/backend/racemod.c) with
...
- The mod store keeps a file open once it has touched it — 32 handles, evicted
  ...
  <code>&lt;root&gt;/store</code>, so every read still goes to the file. What it removes is the
  *open*, and with it the antivirus rescan that a read-after-write pays on a
  player's machine — 890 microseconds against 3.

## What survives a crash

The store under <code>&lt;root&gt;/store/&lt;mod-guid&gt;/</code> holds the only thing this project
...
[`tools/storecrash.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/storecrash.nim) is the evidence rather than the claim. It kills a
...
up above `commitNow` in [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim) so the next person does not
...
<code>&lt;root&gt;/store/&lt;mod-guid&gt;/.hist/&lt;key&gt;.1</code> … `.3`, written at most once per five
...
To use one, stop the server and copy it over <code>&lt;root&gt;/store/&lt;mod-guid&gt;/&lt;key&gt;</code>.
...
**One backend per store.** The server claims <code>&lt;root&gt;/store/.lock-backend</code> on
...
**What a player should back up.** <code>&lt;root&gt;/store/</code> — the whole directory, which
is every mod's saved state and the `.hist` generations with it. Nothing else
under <code>&lt;root&gt;</code> is theirs: `db.json`, `mods/` and the executables all come back
from an install.
...
`aowl test` also runs [`tools/fuzzwire.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/fuzzwire.nim), which drives the same wire with
...
[`backend/framelen.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/framelen.nim) is the second half of that, and it exists because of how
...
gate builds it, stages [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) and `emu-full.json` beside it (the tool
