---
title: "The automation library \u2014 API reference"
---

# The automation library — complete API reference

::: info From the aowlspt repository — [`docs/AUTOMATION-API.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/AUTOMATION-API.md)
...
::: info The current shape of the library
Where [Automation](/docs/aowlspt/automation) still says a piece is under construction, this page is the later record and wins.
:::
...
<code>count &gt; 1</code> on a worn slot is refused by name rather than silently placing the
first.
...
Put [`mods/ammoloading`](https://github.com/aoughwl/aowlspt/tree/main/mods/ammoloading) on a rung: `off` / `probe` / `read` / `spawn` / `animate`.
`probe` proves the trigger fires with zero dereferences and no bundle. The two
rungs above need a bundle riding a vanilla key via [`mods/textures`](https://github.com/aoughwl/aowlspt/tree/main/mods/textures), which this
...
| `applyGear(s)` | every item read back | <code>placed &lt; requested</code>, or a refusal | no backend/profile, route 404, non-200 |
...
|---|---|
| PASS | <code>pact-rpc: QUEUED "&lt;request&gt;"</code> |
...
Asks whether [`mods/ammoloading`](https://github.com/aoughwl/aowlspt/tree/main/mods/ammoloading) **observed** the cycle, asserting on the mod's own
...
| Half | Asked | Answers |
|---|---|---|
| backend (`tarkov.dll`) | `GET /aowlspt/tarkov/autoscript/capabilities` | `{"api":2,"caps":[...]}` — `LoadoutApi` / `LoadoutCaps` in [`mods/tarkov/emu/loadout.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/loadout.nim), beside the code that implements every token |
| host (`aowlspt-host-il2cpp.dll`) | `pact caps` over the inspector channel | `caps api=2 verbs=... readback=... issueonly=...` — `PactCapsLine` in [`host/Aowlspt.Host.Il2Cpp/pact.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/pact.nim) |
