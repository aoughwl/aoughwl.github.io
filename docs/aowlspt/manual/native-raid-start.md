---
title: "Starting an offline raid natively"
---

# Starting an offline raid natively (no menu UI)

::: info From the aowlspt repository — [`docs/NATIVE_RAID_START.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/NATIVE_RAID_START.md)
...
**Instruments.** [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py) (verbs `find` / `type` / `bytes` /
`shared`), [`tools/fldoff.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/fldoff.py) (`fields`, String self-check PASSED:
...
In <code>&lt;OnReadyToStartMatchingAsync&gt;d&#95;&#95;195::MoveNext</code>:
...
| <code>&lt;CurrentTotalRaidNum&gt;</code> | `0x1E4` | incremented by the method itself |
...
|---|---|---|
| <code>&lt;Side&gt;k&#95;&#95;BackingField</code> | `0x20` ✓ (`set_Side@0x6D7FE0` = `89 51 20 C3`) | `ESideType.Pmc = 0` |
| `LocationId` (string) | `0x30` | `"Woods"` via `il2cpp_string_new` |
| <code>&lt;RaidMode&gt;k&#95;&#95;BackingField</code> | `0x44` ✓ (`set_RaidMode@0x6D8720` = `89 51 44 C3`) | `ERaidMode.Local = 1` |
...
<code>&lt;LocalGameMatching&gt;d&#95;&#95;199::MoveNext</code>:
...
<code>Dictionary&lt;string, JsonType.Location&gt;</code> (measured).

Two ways to get `locations["Woods"]`:

* **Preferred — walk the Dictionary's `entries` array by raw offsets** and
  compare the key string in place. Avoids calling <code>Dictionary&lt;K,V&gt;::TryGetValue</code>,
  which IS a shared generic and therefore needs a real `MethodInfo*` we cannot
  fabricate. Cost: the <code>Dictionary&lt;string,Location&gt;</code> instantiated layout is
...
| <code>TarkovApplication::&lt;MainMenu&gt;g&#95;&#95;OnReadyToStartMatching&#124;191&#95;1</code> | `0x988A10` | UNIQUE (1) |
...
| <code>&lt;OnReadyToStartMatchingAsync&gt;d&#95;&#95;195::MoveNext</code> | `0x9CC7B0` | UNIQUE (1) |
| <code>&lt;LocalGameMatching&gt;d&#95;&#95;199::MoveNext</code> | `0x9C3810` | UNIQUE (1) |
...
2. **The <code>Dictionary&lt;string, Location&gt;</code> instantiated layout.** Not reachable
...
* **NULL `MethodInfo*` is correct here** — measured directly from the game's own
  ...
  <code>Dictionary&lt;string,Location&gt;::TryGetValue</code>; §3.3 avoids it by walking.
