---
title: "Backlog \u2014 everything not done"
---

# Backlog: everything this repository says it has not done

::: info From the aowlspt repository — [`docs/BACKLOG.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/BACKLOG.md)
Published from [aoughwl/aowlspt](https://github.com/aoughwl/aowlspt) with only its links repaired. It was written for someone with the checkout open, so it is terse and points at source files by path — [browse the tree](https://github.com/aoughwl/aowlspt/tree/main) as you read. A few of the documents it cites were held back from publication and are not here.

[The manual](/docs/aowlspt/manual/) · [The aowlspt section](/docs/aowlspt)
:::

::: info The unfinished list, frozen
aowlspt is finished and this list was never worked down. It is published as the honest record of what the project knew it had not done — not as a plan.
:::
...
Sources: every `README.md` (root, `registry/`, `installer/`, [`installer/payload/`](https://github.com/aoughwl/aowlspt/tree/main/installer/payload),
[`host/Aowlspt.Overlay/`](https://github.com/aoughwl/aowlspt/tree/main/host/Aowlspt.Overlay), `reference/`, five mods), the other thirteen files in
`docs/`, and the
...
### B4. Two mod READMEs describe gaps in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) that are closed
...
subject could not see them. A gap in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) recorded in
`mods/blackdivision/README.md` is a gap nobody maintaining [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) will
ever re-read. That is why [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) has a README of its own now (G3) rather
...
|---|---|---|
| "The emulator does not read loadouts. [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)'s bot generator reads exactly `firstName`, `health.BodyParts[0]` and `appearance.…[0]` … and nothing else — no `inventory`, no `chances`" | `mods/blackdivision/README.md:133-135` (**struck through in place 2026-08-19**) | [`mods/tarkov/emu/bots.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/bots.nim) rolls `inventory.equipment` per slot against `chances.equipment` and resolves `inventory.mods` breadth-first, and now also fills the rig, pockets and pack from `inventory.items` against the `generation.items` counts. `docs/EMULATOR-COVERAGE.md` records the change. |
| "`/client/game/bot/limit` cannot be changed. [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) answers it with the literal string `30` and it is not backed by any database path." | `mods/morebots/README.md:143-144` (**struck through in place 2026-08-19**) | [`mods/tarkov/tarkov.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/tarkov.nim) reads <code>locations.&lt;loc&gt;.base.BotMax</code> / `BotMaxPvE`, with `gDefaultBotLimit` a setting rather than a literal. |
...
The reason that row mattered more than its size: [`tools/hostharness.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostharness.nim)
...
- `docs/INSTALL.md` — the "not-selected mod is still serving" paragraph is gone;
  ...
  1.2 s sleep.
- [`registry/README.md`](https://github.com/aoughwl/aowlspt/blob/main/registry/README.md) — the "no host answers the control probe" paragraph is
...
derived" section states, over `reference/` and [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov), and joining the two
...
**Done, and it cost an afternoon.** [`tools/coverage.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/coverage.nim) (`aowl-coverage`)
...
`mods/*` have one as of 2026-08-19, and [`mods/manager`](https://github.com/aoughwl/aowlspt/tree/main/mods/manager)'s was the last to be
...
| the whole first-run path | **64** checks (<code>installeruildirstrun.exe --scratch &lt;dir&gt;</code>, re-run 2026-08-19, 0 failed) — a synthetic vanilla client through install, a 39 MiB `importdb`, verify, the boot sequence against real data, live mod control, and a restart with the profile and the selection intact |
...
| gate | checks | how |
|---|---:|---|
| `emutest`, against [`tests/fixtures/emu-full.json`](https://github.com/aoughwl/aowlspt/blob/main/tests/fixtures/emu-full.json) | **596** | <code>emutest.exe --root &lt;stage&gt; --backend … --port &lt;n&gt;</code> |
| `realtest`, against the 39.40 MiB imported database | **168**, 1 skipped | <code>realtest.exe --root &lt;stage&gt; --db build&#92;db&#92;db.json …</code> |
| `soak`, 24 closed play cycles | **915** | <code>soak.exe --root &lt;stage&gt; … --cycles 24</code> |
| `fuzzwire` | **152** | <code>fuzzwire.exe --root &lt;stage&gt; …</code> |
| `wstest`, the notifier websocket | **62** | <code>wstest.exe --root &lt;stage&gt; …</code> |
| `framelen`, hostile `Content-Length` framing | **102** | <code>framelen.exe --root &lt;stage&gt; …</code> |
| `livectl`, live mod control | **213** | <code>livectl.exe --root &lt;stage&gt; …</code> |
...
| `hostharness` against the stand-in runtime | **28** verdicts | <code>hostharness.exe &lt;stage&gt; --runtime tests&#92;mockil2cpp&#92;GameAssembly.dll</code> — count the lines under `Result`, not `grep -c '^ok'`, which answers 31: three of those are the harness reporting its own staging before it has read a line of the host's log. It was 24 until 2026-08-19, when four verdicts about an instance method's unreportable fourth argument landed |
| `ws_stall`, a websocket peer that stops reading | **15** | <code>tests&#92;ws&#95;stall.exe --port &lt;n&gt;</code> — 14 and **2 failures** against the engine before the poller stopped waiting on a worker |
| `storeguard`, a reader holding a key across a commit | **9** | <code>installer&#92;build&#92;storeguard.exe --root &lt;dir&gt;</code> — 2 fail against the store as it was, because a handle that never sees a change is trivially never torn |
...
| `ovsync`, the client host's poll reaching the backend | **24** | <code>ovsync.exe --root &lt;dir&gt; --backend … --manager … --port &lt;n&gt;</code> — 18 fail against a dead port, with `silentPolls: 0` as the proof nothing arrived |
...
**[`tools/firstrun.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/firstrun.nim) runs, and the row above is measured rather than
...
| S14 | **[`mods/sain`](https://github.com/aoughwl/aowlspt/tree/main/mods/sain)'s two postfix hooks, which have never fired** | `mods/sain/README.md:241-245` | The stand-in has none of the six candidate method names, so both refuse and both cost lines read "nothing measured". Same position as `mods/classicmovement`'s `tiltCostNs`. |
...
- **Generic instantiations have no name `findClass` accepts** — the reason SAIN
  cannot read bleeds and fractures, and the reason <code>List&lt;IPlayer&gt;</code> chains are out
...
and `--loose all`, which was blamed on `jsondb` and is a route in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov).
...
| **`staticAmmo` is imported and unread** | `docs/IMPORTDB.md:132-136` | Explicitly *named as not expressible*: the table imports fine, and nothing consumes it — `expandAmmoBox` picks from the box template's own filter instead. *"The data is imported so that the day someone wires it up, the table is already where they would look."* **Re-checked 2026-08-19: still true.** `staticAmmo` appears 13 times in `build/db/db.json` (one per map that has one) and [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) names it only in comments and in `raid.nim`'s check that it is *stripped* from `/client/locations`. | A day in [`mods/tarkov/emu`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov/emu). |
| **Seventeen file groups not imported** | `docs/IMPORTDB.md`, "The last row, itemised" | This row said *"nine tables … ~9 MB"* and **both halves were wrong.** The nine were real, and **eight more file groups nobody had listed** were in the etcetera at the end of it: `locations/*/statics.json`, `locations/*/allExtracts.json`, Ragman's `suits.json` / `bearsuits.json` / `usecsuits.json`, `templates/archivedQuests.json`, `templates/customAchievements.json`, `templates/customisationStorage.json`, `traders/*/services.json` and `server.json`. Measured on `D:\SPT` on 2026-08-19: **11,246,992 bytes (11.25 MB) across 71 files that nothing reads**, plus **50,423,639 bytes (50.42 MB) of other-language locales**, which are not a gap at all — they are one `--locales` flag away. Reason given for the rest is still *"the emulator has no route that reads them"*, which makes it a consequence of Group F rather than a cause. **Re-checked: still true**, and the importer now says so mechanically — `checkReadPaths` (`tools/importdb.nim:669`) walks the 29 paths [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) reads with a literal spelling and asserts each against the document it just wrote, instead of a sentence asserting it. | Follows the routes. |
| ~~**Trader healing (`RestoreHealth`)**~~ | [`mods/tarkov/emu/health.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/health.nim) | **Done, and it was never blocked.** This row said *"needs the treatment price table"* and named an importer change. The table was in the database the whole time: `globals.config.Health.HealPrice` and <code>Effects.&lt;name&gt;.RemovePrice</code>, times the trader's `loyaltyLevels[n].heal_price_coef`. Nothing had to be imported. | Was: importer row. Actually: nobody had looked. |
| ~~**`--loose all` makes one response 560 MB**~~ **— fixed 2026-08-19** | `mods/tarkov/emu/raid.nim:40-110`, `:403-460`; was `docs/IMPORTDB.md`, *Why loose loot is opt-in* | **Done, in the place this row said it belonged.** `/client/locations` no longer splices the `locations` subtree in verbatim: `raid.nim` builds the map list from each map's `_Id` → `base` and nothing else — which is exactly what `LocationsGenerateAllResponse` asks for — with the key list cached after the first request, and `paths` carried through when the database has one. `base` still goes out byte for byte, so `morebots`' and `icebreaker`'s writes into <code>locations.&lt;map&gt;.base</code> still arrive. The absence is asserted rather than assumed: `raid.selfCheckRaid` runs the builder over a literal fixture that deliberately carries a map's `looseLoot` and a `staticAmmo` table and fails the load if either appears in the body, and it runs at load out of `emu/selfchecks.nim:133` — `emutest`'s own `/aowlspt/tarkov/selfcheck` check is green in the 541-check run above. **What this row used to say is kept:** the reason *it* gave was already a retirement of an earlier one — it said the problem was `jsondb`, *"it is held as one text document, indexed by byte offset, and `dbWrite` replaces the whole thing"*, and that had been fixed (a `db_patch` of an existing value went from **171 ms to 6.1 ms**); what actually made loose loot opt-in was this route. Measured 2026-08-19, before the fix: `aowl-importdb --from D:\SPT --loose all --no-check` produces **616,032,908 bytes (587.49 MiB)** in 13.1 s, and one <code>aowlprobe http://127.0.0.1:&lt;port&gt;/client/locations</code> against a backend on it returned a **560 MB body** (19.1 s wall including writing it out; `docs/IMPORTDB.md` measures 10.8 s server-side). Nothing failed then either — `realtest` passed all 134 checks against that database. **The 560 MB has not been re-measured against the new route**; what has been measured is that the loot is gone from the body. | Was: a route change in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov). Done. |
| **31 templates render as their id** | `docs/IMPORTDB.md` | No <code>&lt;id&gt; Name</code> in the English locale. *"ugly and not broken"*. **Re-checked 2026-08-19** by re-running the import: *"templates with a localised name: 4642 of 4673"* — 4673 − 4642 = 31, still exactly 31. | Source data. |
...
**In [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) — the emulator's own surface**
...
- **Real content gaps, ~21** — down from ~26, and *this group's reasons were
  ...
  **The standing instruction, and it is the whole lesson of this section:
  ...
  |---|---|---|
  | `RestoreHealth` | "the treatment price table" | `globals.config.Health.HealPrice` and <code>Effects.&lt;name&gt;.RemovePrice</code>, times `loyaltyLevels[n].heal_price_coef` |
  ...
  **Still genuinely blocked, each re-checked against `build/db/db.json` on
  ...
  | `GetDialogue` | `templates/dialogue.json`, 6,259,288 bytes, unimported. **Not** <code>traders/&lt;id&gt;/dialogue.json</code>, which is imported and *is* read now | **yes** |
...
- **Five *(unverified)* wire shapes in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)** —
...
**In [`mods/sain`](https://github.com/aoughwl/aowlspt/tree/main/mods/sain) — sensors**
...
- ~~**No gate for the client half of live mod control**~~ — **closed
  ...
  [`tools/livectl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/livectl.nim) (**213 checks**) and both ends of the *wire* by
  ...
  [`tests/ovsync`](https://github.com/aoughwl/aowlspt/tree/main/tests/ovsync) is that gate: **24 checks**, in `aowl test`. Nothing in the
  loop is stood in for — a real backend on its own root and port, the real
  worker thread reached through [`host/Aowlspt.Overlay/aowloverlay.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Overlay/aowloverlay.nim) (the
  ...
  **Three things it found that the documents had wrong**, all now corrected in
  ...
  1. [`abi/aowlspt_overlay.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_overlay.h) said `Accept-Encoding: identity` *"is ignored by
...
- ~~**The client host never reports its outcome back to the manager**~~ —
  **closed on both sides.** The host reports on the poll it already makes
  ([`host/common/modcontrol.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modcontrol.nim), proved by [`tests/modreport`](https://github.com/aoughwl/aowlspt/tree/main/tests/modreport), 27 checks) and the
  manager reads it ([`mods/manager/mgr/clientreport.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/clientreport.nim), proved by
  [`tests/modclient`](https://github.com/aoughwl/aowlspt/tree/main/tests/modclient), 90 checks); both are in `aowl test`. `live` on a
...
- ~~**A mod's DLL can be freed while a worker is inside its route handler**~~ —
  **closed in code, and the remaining work is that nothing in `aowl test` runs
  ...
  half was already closed (retired, never freed, [`tests/detour_race.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_race.c) in the
  gate); the module half is [`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) — `aowl_mod_inflight` /
  ...
  [`backend/modrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/modrace.nim), 712 lines with a vectored fault handler, a generation
  ...
  **What is open is one line of [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim).** `grep -n modrace
...
- ~~**The 14-byte prologue write is not atomic**~~ — **closed 2026-08-19.**
  ...
  [`tests/detour_race.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_race.c) the whole time. Measured, six saturating workers:
  ...
  **The rate this row used to quote — "~1 in 140,000 calls" — is not
  ...
  for the real engine and it is much larger: [`tests/detour_race.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_race.c) records that
...
- ~~**No `everyMain`**~~ — **closed.** `everyMain`/`stopMainRepeats` are in
  [`aowl/src/aowlspt.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt.nim): a repeating callback on the drain's thread holding
...
- **No automated store rollback** — `host/common/modstore.nim:86-88` keeps a
  three-generation `.hist` ring and there is no revert path in [`host/common/`](https://github.com/aoughwl/aowlspt/tree/main/host/common).
...
- **[`mods/sain`](https://github.com/aoughwl/aowlspt/tree/main/mods/sain) will not guess `EnemyLookAtMe`, will not infer bleeding from
...
- ~~**Sixteen detour thunk slots**~~ — **256 now**, and the old number was not a
  ...
  **One hook per method** is the limit that actually bites, and it is
  ...
  in `docs/MODDING.md` and at the top of [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson).
...
following the unlinked file forever. [`tests/storeperf/storeguard.nim`](https://github.com/aoughwl/aowlspt/blob/main/tests/storeperf/storeguard.nim) fails 2 of
...
`registry/validate.nim` and [`mods/manager/mgr/registry.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/registry.nim)). Nothing is left
...
wording was also stale: [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) is **37 modules under `emu/` plus
`tarkov.nim`**, not 33.
...
gaps in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) were recorded only in *other mods'* READMEs, and nobody
maintaining [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) was going to see them go stale. `mods/sway` had no gap
...
2. **The broken section went from nine to three and is now empty.** The last
   ...
   place, and the hand-maintained table became [`tools/coverage.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/coverage.nim), which
...
3. **The two items this list called "one call site from closed" are done, and
   ...
   *runtime's own counters* rather than against the mod's own report — because
   ...
   the module refcount: `modEnter` landed, and [`backend/modrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/modrace.nim) shows the
...
| **The overlay legend was cut in half at the 480 floor** | 57 columns for a 106-character legend, silently truncated -- "loses the marker nobody knows yet", in the header's own words. [`tests/overlayhost`](https://github.com/aoughwl/aowlspt/tree/main/tests/overlayhost) decodes the back buffer into characters now, so the check is about pixels rather than intent |
...
and the six examples. The two worth naming: **[`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) §9 taught that
...
nowhere. [`tools/firstrun.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/firstrun.nim) runs again -- 64 checks -- and now halts at the
...
subject.* [`mods/fov`](https://github.com/aoughwl/aowlspt/tree/main/mods/fov) was cited to two READMEs as evidence that a hook is given
its instance; it never calls `thisPointer` and still printed the false claim.
The fix for that class is not more prose -- it is [`tools/coverage.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/coverage.nim),
...
| The notifier is a poll, not a websocket | a real websocket, poll kept as fallback | `backend/websocket.nim:312` (handshake), `:277` (`wsPush`); gate [`tools/wstest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/wstest.nim) on port 6982 |
| No SHA-256 in the toolchain | implemented | `tools/release.nim:144`, and in `registry/validate.nim` and [`mods/manager/mgr/registry.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/registry.nim) |
| The registry repository does not exist | it does | [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json), `validate.nim`, `fakeregistry.py`, plus [`tools/regcheck.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/regcheck.nim) as a gate |
...
| The client host never reports its outcome back to the manager | closed on both sides | [`host/common/modcontrol.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modcontrol.nim) (proved by [`tests/modreport`](https://github.com/aoughwl/aowlspt/tree/main/tests/modreport), **27** checks) and [`mods/manager/mgr/clientreport.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/clientreport.nim) (proved by [`tests/modclient`](https://github.com/aoughwl/aowlspt/tree/main/tests/modclient), **90** checks); both re-run 2026-08-19 |
| A mod's DLL can be freed while a worker is inside its route handler | a refcount around the dispatch, with a drain and a deadline | `host/common/modhost.nim:210` (the C storage), `:273` (`modEnter`), `backend/aowlbackend.nim:936` and `:1117`. Reproducer [`backend/modrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/modrace.nim): guarded, 43393 answers and no fault across 40 unloads under 8 threads; `--unguarded`, 8 of 8 workers fault. **The proof is not in `aowl test` — see the open row above.** |
...
literal in the client. `docs/WIRE.md` is the description; [`tools/bsgwire.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/bsgwire.py)
and [`tools/bsgaes.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/bsgaes.py) are the implementation. All 171 encrypted bodies in
...
Fixed in [`backend/wire.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/wire.nim) and `aowlbackend.nim`; gated by
[`tests/bsgwiretest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tests/bsgwiretest.nim) against the real bodies.
...
keys -> six; `/client/quest/list` object -> array, with [`tools/realtest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/realtest.nim)'s
...
writing.** Both waited for <code>listening on 127.0.0.1:&lt;port&gt;</code>; the backend writes
<code>listening on &lt;scheme&gt;://127.0.0.1:&lt;port&gt;</code>, and has since it learned to serve
