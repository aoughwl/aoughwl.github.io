---
title: "Mod Settings API \u2014 design"
---

# Mod Settings API — design

::: info From the aowlspt repository — [`docs/MOD-SETTINGS-API.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/MOD-SETTINGS-API.md)
...
::: warning Design only — never built
This is a design document for an API that was not implemented. The settings API that did ship is [the HTTP one](./ui-api), and the reader-facing description is [Configuration](/docs/aowlspt/configuration).
:::
...
1. **[`aowl/src/aowlspt/settings.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/settings.nim)** — a mod-facing schema
...
2. **[`host/Aowlspt.Host.Il2Cpp/settingspages.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/settingspages.nim)** (929 lines) — the ONLY
...
3. **[`mods/settingshub/settingshub.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/settingshub/settingshub.nim)** — the aggregation pattern to copy:
   `GET /aowlspt/settings/spt/index` → page list, <code>GET /aowlspt/settings/spt/page/&lt;id&gt;</code> → one page. Built for the SPT-config
...
default is `SelectSlider` when <code>options.len &lt;= 6</code>, else `DropDown` — a
...
per-mod <code>/aowlspt/settings/&lt;guid&gt;</code> convention (`matchRoute`'s exact-match /
...
`/index` + <code>/page/&lt;id&gt;</code> shape). It needs a way to enumerate every OTHER mod's
...
`SettingType` (<code>bool-&gt;stBool</code>, <code>int-&gt;stInt</code>, <code>float-&gt;stFloat</code>, <code>string -&gt;stString</code>, <code>enum-&gt;stEnum</code> with `options` from `$typ` iteration) and reads
...
POST to <code>/aowlspt/settings/&lt;guid&gt;</code>. **No new persistence layer.** The
...
- [ ] Replace `swBuildHostPage()`'s hard-coded host-flag list with an HTTP
      fetch of `GET /aowlspt/settings/index` then one <code>GET /aowlspt/settings/&lt;guid&gt;</code> per mod, building one `SwPage` per mod (or
      grouping by `category` within the tab — undecided, see Open
      Questions).
- [ ] Wire `swReadBackPage`'s existing write path to <code>POST /aowlspt/settings/&lt;guid&gt;</code> instead of (or alongside) the current
...
- Each mod also self-registers into a shared registry proc exported from
  ...
  or
- The aggregator does N internal HTTP calls to each <code>/aowlspt/settings/ &lt;guid&gt;</code> it already knows about (from [`mods/manager`](https://github.com/aoughwl/aowlspt/tree/main/mods/manager)'s existing mod list —
...
3. **SelectSlider vs DropDown heuristic** (<code>options.len &lt;= 6</code>) is a guess,
