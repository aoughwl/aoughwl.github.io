---
title: "SAIN driver \u2014 the RVA table"
---

# SAIN driver â€” the RVA table, and the one live measurement that gates it

::: info From the aowlspt repository — [`docs/SAIN_RVA.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/SAIN_RVA.md)
...
**The byte-verified static address table that let the bot driver stop resolving game methods by name at runtime.**
...
the **startup snapshot** ([`abi/aowlspt_prologue.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_prologue.h)), never live memory.
...
|---|---|---|---|---|
| getter | <code>EFT.BotOwner::get&#95;Mover() -&gt; BotMover</code> | `0x80F920` | UNIQUE | (getter) |
| getter | <code>EFT.BotOwner::get&#95;Steering() -&gt; BotSteering</code> | `0x80D8E0` | UNIQUE | (getter) |
| getter | <code>EFT.BotOwner::get&#95;WeaponManager() -&gt; BotWeaponManager</code> | `0x80F040` | UNIQUE | (getter) |
| getter | <code>EFT.BotOwner::get&#95;AIData() -&gt; IAIData</code> | `0x810360` | UNIQUE | (getter) |
| getter | <code>AIData::get&#95;BotOwner() -&gt; BotOwner</code> | `0x692A50` | **SHAREDÃ—338** trivial `[rcx+0x28]` | call-safe; = field @0x28 |
| move | <code>BotMover::GoToPoint(Vector3,bool,float,bool,bool,bool,bool) -&gt; NavMeshPathStatus</code> | `0x1A2EE00` | UNIQUE | `48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 70 48` |
...
| reload | <code>BotReload::TryReload() -&gt; bool</code> | `0xBB44A0` | UNIQUE | `40 53 48 83 EC 30 33 C0 48 8B D9 48 89 44 24 40` |
...
unconditionally and needed a live <code>List&lt;Player&gt;</code> measurement a subagent could not
...
- `mods/sain/client/live.nim:2029-2085` -- `listLen`/`listAt`/`arrayOf`/`listBound`
  read the fixed il2cpp <code>List&lt;T&gt;</code>-of-reference layout (`_items` object[] @0x10,
...
- [`mods/sain/client/drivecalls.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/client/drivecalls.nim) calls `BotMover::Sprint` @0x1A2D700,
  ...
  byte-verified against [`abi/aowlspt_symtab.nim`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_symtab.nim). All seven re-measured
...
- The component hops are measured static field offsets on the non-generic
  `EFT.BotOwner` (<code>&lt;Mover&gt;@0x3d0</code>, <code>&lt;Steering&gt;@0x148</code>, <code>&lt;ShootData&gt;@0x280</code>,
  <code>&lt;Medecine&gt;@0x2c8</code>), not getter calls, so no shared getter is involved.
...
`BotMover::SetTargetMoveSpeed` @0x1A2B4D0 and the [`abi/aowlspt_botnav.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_botnav.h)
thunk) as the single writer, with the client actuator's refusals COUNTED
(`sprintWithheld`) rather than silenced. See [`mods/sain/config.json`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/config.json)
...
2. Each converted bind INSTALLS a live read/call into game code. A wrong
   ...
   requires the live diag <code>K &gt; 0</code> plus a per-setting read-back, which only the
...
verified live against <code>K&gt;0</code> and a value read-back before the next is added.
...
Both were in [`mods/sain/sain.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/sain.nim)'s damage-hook comment rather than here, but
...
The registered settings ([`mods/sain/sain.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/sain.nim), 41 rows) are almost all
...
<code>sain: applied &lt;key&gt;=&lt;v&gt;, bot read back &lt;v&gt;</code> shows a live bot value that CHANGES
