---
title: "Bot navigation \u2014 IL2CPP recon"
---

# Bot navigation / AI control — IL2CPP recon

::: info From the aowlspt repository — [`docs/BOTNAV.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/BOTNAV.md)
...
the decrypted `global-metadata.dat`, using [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py) (type/field/RVA)
...
* `StubAIData` (type 1524): <code>&lt;BotOwner&gt;k&#95;&#95;BackingField</code> @ **+0x30**
...
| 0x130 | <code>&lt;Sprinting&gt;</code> bool |
| 0x138 | <code>&lt;IsMoving&gt;</code> bool |
| 0x148 | <code>&lt;SDistDestination&gt;</code> float |
| 0x158 | <code>&lt;Pause&gt;</code> bool |
| 0x15C | <code>&lt;MoveSpeed&gt;</code> float |
| 0xD8 | `_pathFinder` (`BotPathFinderCorePoints`) |

`BotMoverStateMachine`: `_owner`@0x10, <code>&lt;CurrentMoverState&gt;</code>@0x18,
`_states` (<code>Dictionary&lt;EBotMoverState,BotMoverState&gt;</code>) @ **0x20**.
...
4. **Passing <code>reachDist &gt;= 0</code> skips the `Settings` pointer chain entirely.** With
...
`LocationBase` as we serve it (see [`mods/tarkov/emu/raid.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/raid.nim),
...
(<code>Dictionary&lt;PatrolPoint,BotZone&gt;</code> @ +0x30). They are never server data, so there
...
`_zonesPatrols`@0x30, `_allPlayers`(<code>List&lt;Player&gt;</code>)@0x48, `_allBotsCount`@0x70,
`OnBotCreated`(<code>Action&lt;BotOwner&gt;</code>)@0xA8, `OnBotRemoved`@0xB0, `MaxBots`@0xC0,
...
doc key (written by [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim)'s config stager) and a row on the in-game
...
`aowl_pro_verify` ([`abi/aowlspt_prologue.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_prologue.h)), i.e. against the ORIGINAL bytes
...
| File | What |
|---|---|
| [`abi/aowlspt_botnav.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_botnav.h) | offsets, the verified target table, guarded readers, and the three direct-call thunks |
| [`host/Aowlspt.Host.Il2Cpp/botnav.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/botnav.nim) | registry, command grammar, service tick (`include`d into `aowlhost.nim`) |
| [`host/common/modcontrol.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modcontrol.nim) | `parseBotNav` (intake gate), `reportBotNav` + census on `reportPath` |
| [`mods/manager/mgr/control.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/control.nim) | `aowlspt.bot.nav` intake, `aowlspt.bot.census` emit |
| [`mods/manager/mgr/clientreport.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/clientreport.nim) | census ingest off the poll's query string |
| [`aowl/src/aowlspt/botnav.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/botnav.nim) | the mod-facing API |
...
* `reachDist` is forced <code>&gt;= 0.25</code>, so the `Settings` pointer chain is never
...
6. The log prints <code>botnav: bot id=N GoToPoint(x, y, z) -&gt; Complete</code> and the
...
`placeLibrary`'s job — the artifact is left in <code>nimcache/&lt;hash&gt;/aowlhost.dll</code>),
