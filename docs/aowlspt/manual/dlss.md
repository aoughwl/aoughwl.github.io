---
title: "DLSS on this install"
---

# DLSS on this install -- what works, what does not, and the evidence

::: info From the aowlspt repository — [`docs/DLSS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/DLSS.md)
...
::: info One machine, one day
These are measurements of one specific GPU on one specific build. They are evidence about that install, not a general statement about the feature.
:::
...
with its source. The tool is [`tools/dlss.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/dlss.py); its self-test is registered in
[`tools/selftests.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/selftests.py).
...
* **"DLSS 5" is not a Super Resolution upgrade and is not installable here.**
  ...
  anyway, knowingly** -- that is [`tools/dlssnr.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/dlssnr.py) and `docs/DLSSNR.md`, a
...
to <code>.cache/dlss/backup/&lt;ts&gt;/</code>; copies the new DLL beside the old one and `os.replace`s it
...
2. Start the client. Read <code>D:&#92;Aowlspt&#92;Logs&#92;&lt;ts&gt;&#92;&#42;files-checker&#95;000.log</code>: it must say
...
[`tools/dlss.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/dlss.py) is where a pinned `nvngx_dlssnr.dll` goes; the manifest/backup/verify
machinery is feature-agnostic.

**Superseded 2026-09-03 by an explicit user decision**: the leaked route IS installed
here, eyes open, through [`tools/dlssnr.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/dlssnr.py) -- see `docs/DLSSNR.md` for the measured state
...
* The client's REAL consistency manifest is an AES blob appended to `EscapeFromTarkov.exe`
  (see [`tools/consistency.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/consistency.py)); the plain `ConsistencyInfo` is the launcher's copy. `dlss.py
