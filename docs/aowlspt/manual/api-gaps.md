---
title: "Three gaps in the mod API"
---

# Three gaps in the mod API, and what closing them would cost

::: info From the aowlspt repository — [`docs/API-GAPS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/API-GAPS.md)
...
**Three features `mods/pathtotarkov` wanted and the mod API could not express, each written up as a fix with its cost.**
...
| | gap | cost | value | rank |
|---|---|---|---|---|
| 1 | `tarkov.raid.ended` carries only the profile | one broadcast payload in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov); **no ABI change** | two mod features that are impossible today | **first** |
...
| 2 | no `dbKeys(path)` | index change in [`backend/jsondb.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/jsondb.nim) + a host entry or a path form; possibly ABI revision 6 | every mod that iterates a table stops shipping a hard-coded key list | third |
...
([`abi/aowlspt_abi.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_abi.h), the full entry list ends at `notify_push` on line 561).
The mapping exists, but it is [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)'s private store:
...
**Who owns the mapping decides this.** [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) binds sessions because it
...
2. **A bind event.** [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) broadcasts `tarkov.session.bound`
...
3. **A relay, for mods that would rather not keep a copy.** [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)
...
`broadcast` and `onEvent`, both revision 1. The work is inside [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)
...
`notify_push` was added for, and which today only [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) can do. Concretely:
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) measured what that is: **12.5 MB and 247 ms on a default import,
...
prints what it cost. [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) pays the read exactly once and caches
...
### What it would actually cost in [`backend/jsondb.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/jsondb.nim)
...
way (`abi/aowlspt_abi.h:576-580`) and [`tests/abi_layout.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/abi_layout.c) pins them to the
real offsets.
...
Mod-side wrapper, in [`aowl/src/aowlspt/server.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/server.nim) next to `dbRead` (`:133`):
...
* [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) deletes its `LocationIndex` cache and the restart-to-see-a-new-map
...
2. **Gap 3 — sessions on events.** Nearly free, and its cheap half is
   *literally the same edit* as gap 1: put `session` on the two raid
   broadcasts. The bind/unbind events are twenty more lines in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov).
