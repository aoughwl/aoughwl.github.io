---
title: "Decrypting global-metadata.dat"
---

# Decrypting `global-metadata.dat`

::: info From the aowlspt repository — [`docs/METADATA-DECRYPT.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/METADATA-DECRYPT.md)
...
Escape from Tarkov build, for use with [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py).
...
* The real game install (here `D:\Aowlspt`). Never copy
  ...
  output is not.)
* An installed layout blob at <code>mods/tarkov/data/metadata/&lt;version&gt;.json</code>.
  Check with `status`; if missing, obtain it with
  <code>python tools/metablob.py extract-capture &lt;gamedir&gt; &lt;capture.txt&gt;</code>
  (see [`tools/METABLOB.md`](https://github.com/aoughwl/aowlspt/blob/main/tools/METABLOB.md)).
...
Note `find` matches **type** names, not method names; use <code>type &lt;idx&gt;</code> to list
