...
::: info From the aowlspt repository — [`docs/INSTALL.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/INSTALL.md)
...
This is the order somebody actually walked, not an idealised one. [`tools/firstrun.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/firstrun.nim)
...
| | you extracted <code>aowlspt-&lt;version&gt;.zip</code> | you cloned the repository |
|---|---|---|
...
back. Nothing else versions the bundle — the numbers in [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json)
...
[`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) — the mod manager would have nothing to manage — and a
...
install root**: the backend is started with <code>--root &lt;target&gt;&#92;aowlspt</code> and
reads <code>&lt;root&gt;&#92;db.json</code>. Put it one level up and the server comes up on its
fallbacks with nothing anywhere saying why.

The importer opens the SPT install read-only and refuses an `--out` inside it.
[IMPORTDB.md](./importdb) has what converts, what does not, what the self-check
...
polls <code>127.0.0.1:&lt;port&gt;</code> until the backend answers a request — a socket that
...
why `aowlprobe` exists. See [registry/README.md](https://github.com/aoughwl/aowlspt/blob/main/registry/README.md) for the
...
naming an empty `load`. [MODMANAGER.md](./modmanager) has why the last one is a
...
many lists it found, and [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) names them.
...
| the whole first-run path, in this order | [`tools/firstrun.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/firstrun.nim): from a synthesised vanilla client through payload, install, `importdb`, `verify`, the client's boot sequence against a 39 MiB imported database, a mod switched off and on live, and a restart with the profile still in it |
