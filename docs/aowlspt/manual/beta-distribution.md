---
title: "Beta distribution and IP protection"
---

# Beta distribution & IP protection — the free public beta

::: info From the aowlspt repository — [`docs/BETA-DISTRIBUTION.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/BETA-DISTRIBUTION.md)
...
**The design for a free public beta and the licence gate behind it.**

Design pass, with one implemented, tested core ([`tools/betagate.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/betagate.nim)). Every
...
A symmetric MAC (HMAC/SHA-256, which [`tools/release.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/release.nim) already has) is
...
[`abi/aowlspt_shim.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_shim.h), called the same way the TLS shim in [`abi/aowlspt_tls.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_tls.h)
...
genuinely tricky part — is implemented and unit-tested in [`tools/betagate.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/betagate.nim)
...
- a value under <code>HKCU&#92;Software&#92;Aowlspt&#92;b&#92;&lt;archiveId&gt;</code> (name obfuscated),
...
Extend the existing, reproducible [`tools/release.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/release.nim) (do not fork it):
...
4. **Refuse to release** (reuse the `deploy.json` marker discipline —
   ...
   host/backend/launcher does not match the signing key, or <code>expiryEpoch &lt;= buildEpoch</code>. A verification that cannot fail is the bug; each of these catches
...
## 6. What is implemented in this pass: [`tools/betagate.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/betagate.nim)
...
calls, and unit-tested ([`tests/test_betagate.nim`](https://github.com/aoughwl/aowlspt/blob/main/tests/test_betagate.nim), run with the standard `nim`
compiler). It exposes:
...
- no input combination yields "valid" once <code>effectiveNow &gt;= expiry</code>.
...
2. Vendor the reviewed Ed25519 verify behind [`abi/aowlspt_shim.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_shim.h); bake the
   public key into host + backend + launcher as a constant.
3. Port [`tools/betagate.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/betagate.nim) into a shared host/backend module; add the OS
...
5. Extend [`tools/release.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/release.nim) with `--beta` (§5) and its refuse-to-ship gates.
