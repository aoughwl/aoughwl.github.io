---
title: "Mod-to-mod capabilities"
---

# Mod-to-mod capabilities

::: info From the aowlspt repository — [`docs/CAPABILITIES.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/CAPABILITIES.md)
...
**How one mod offers a named service and another mod finds and calls it, server-side, without either one knowing the other exists.**

[`aowl/src/aowlspt/capability.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/capability.nim). Server side. The module header carries the
...
1. `provide(name, version, handler)`. From `onLoad` or any later point, on any
   thread.
2. Add the name to your `provides` array in [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json), versioned
...
deliberately loaded first ([`tools/capproof.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/capproof.py), backend log):
...
### [`mods/admin`](https://github.com/aoughwl/aowlspt/tree/main/mods/admin) — the measured victim
...
what [`examples/capprovider`](https://github.com/aoughwl/aowlspt/tree/main/examples/capprovider) accepts and what a `provide` on [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) would
take. The change is:
...
[`mods/tarkov/emu/spawn.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/spawn.nim), plus `"items.spawn/1"` in aowl.tarkov's `provides`
— belongs to whoever owns [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov). Until it exists, admin's refusal
becomes `coNoProvider` with the registry named, which is strictly better than
today's static sentence.

### [`mods/waypoints`](https://github.com/aoughwl/aowlspt/tree/main/mods/waypoints) — the second instance
...
where the handler takes <code>&#123;"map":"&lt;id&gt;"&#125;</code> and returns the data it already
