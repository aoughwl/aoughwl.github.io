---
title: "Getting past character-select"
---

# Getting past character-select: the host does it, not the launcher

::: info From the aowlspt repository — [`docs/AUTOENTER.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/AUTOENTER.md)
...
**Who answers Tarkov's character/mode screen when the game starts — the host, natively, rather than the launcher driving the UI.**
...
**The host path (default).** [`host/Aowlspt.Host.Il2Cpp/modeskip.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/modeskip.nim), gated by
...
**The launcher path (`--auto-enter`).** [`tools/aowllaunch.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowllaunch.nim) waits for the
`CharacterSelectionScreen` to be up and presses the "PvE" slot through the
live-inspector file channel, using [`tools/aowlui.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowlui.nim). Keep it for debugging
...
* **Every batch names its writer.** Sentinels are
  <code>aowl-batch-launch&lt;pid&gt;-&lt;ms&gt;-&lt;n&gt;</code>, the convention
  [`plugins/aowlsptcode/mcp/channel.py`](https://github.com/aoughwl/aowlspt/blob/main/plugins/aowlsptcode/mcp/channel.py) uses. The sentinel is the only part of a
  batch that reaches the host log (the host echoes <code>&gt; echo aowl-batch-...</code> and
...
`aowlspt-inspect.txt` for an <code>aowl-batch-launch&lt;pid&gt;-</code> sentinel.
