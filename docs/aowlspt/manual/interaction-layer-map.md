---
title: "The interaction layer"
---

# The interaction layer — how the aowlspt host touches the live client

::: info From the aowlspt repository — [`docs/INTERACTION-LAYER-MAP.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/INTERACTION-LAYER-MAP.md)
...
**The complete map of the seam where the host touches the running client — the layer every crash of early September came out of.**
...
([`abi/aowlspt_il2cppready.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_il2cppready.h)).
...
### 1.4 The main-thread queue contract — [`abi/aowlspt_mqpolicy.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_mqpolicy.h)
...
`since_ms` is consulted **only once <code>fires &gt; 0</code>** — before the first firing the
...
The header is deliberately free of <code>&lt;windows.h&gt;</code> and of every host type so that
[`tools/test_mqpolicy.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/test_mqpolicy.py) can **compile this file with a tiny main and execute the
...
because <code>MI&#95;DEBUG &gt; 0</code> — and `include/mimalloc/types.h:69-75` defaults `MI_DEBUG`
to **2** unless `MI_BUILD_RELEASE` or `NDEBUG` is defined, which nimony only does
under `-d:release`/`-d:danger`, which [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) never passed. **Every DLL
we had ever shipped linked a debug allocator that turns a heap anomaly into an
instant client kill.** Fixed by `mimallocFlags` in [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) and the
...
[`host/Aowlspt.Host.Il2Cpp/aowlhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/aowlhost.nim), a plain module-level Nim string assigned
...
[`backend/aowlbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/aowlbackend.nim) had declared its identically-named `gLastError`
...
| P3 | `aowl_mq_may_run_here` is the only predicate deciding where queued work runs, and there is one copy | [`tools/test_mqpolicy.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/test_mqpolicy.py) (compiles + runs the real function) |
| P4 | "bound but never fired" is never reported or treated as a stall | [`tools/test_mqpolicy.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/test_mqpolicy.py) |
| P5 | no `exportc` proc takes or returns a Nim `string`/`seq` | [`tools/abilint.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/abilint.py) check 1 |
| P6 | no module-level ARC `var` is assigned inside an `exportc` body unless `{.threadvar.}` or locked | [`tools/abilint.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/abilint.py) check 2 |
| P7 | no shipped binary contains mimalloc's assertion strings (`MI_DEBUG=0`) | [`tools/allocasserts.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/allocasserts.py) |
...
## 2. The detour engine — [`abi/aowlspt_detour.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_detour.h)
...
* [`tools/drainaudit.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/drainaudit.py) — the **offline** half. It refuses the *build*, so the bad
...
unbounded blast radius. <code>R shared &lt;RVA&gt;</code> has **three** outcomes — `shared` /
...
| id | predicate | enforcement |
|---|---|---|
| D1 | no postfix site in `host/` or `mods/` exceeds 4 register slots | [`tools/drainaudit.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/drainaudit.py) (build-gating) + `attachDrain` + `invoke.nim:postfixRefusal` |
| D2 | every postfix/drain site declares a slot count agreeing with the metadata **by RVA** | [`tools/drainaudit.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/drainaudit.py) |
...
| D10 | every feature is flag-gated and defaults OFF | [`tools/hostcfg.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostcfg.py) validates key *names* only; the default is **UNENFORCED** |
...
### 3.3 The handle shape filter — [`abi/aowlspt_handle.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_handle.h)
...
permission to call one without.* [`tools/test_handleshape.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/test_handleshape.py) compiles and runs it
...
Every input is available **offline**, so [`tools/il2cpp_nameindex.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_nameindex.py) walks the
...
| X3 | the shape filter accepts every real pointer shape ever observed | [`tools/test_handleshape.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/test_handleshape.py) case 0 |
...
Offsets come from `Il2CppMetadataRegistration.fieldOffsets` via <code>R fields &lt;Type&gt;</code>
or [`tools/fldoff.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/fldoff.py), each with a mandatory self-check
...
**Instantiated generic layouts (<code>List&lt;int&gt;</code>) are not reachable offline** — all
...
| M9 | UI text goes through the real setters and is re-applied | [`tools/acceptance.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/acceptance.py) (behavioural, live) |
...
| I2 | search verbs report EXHAUSTIVE vs STOPPED EARLY and never convert the latter into absence | [`tools/test_inspect_silence.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/test_inspect_silence.py), recorded `toolcheck` cases |
...
| I6 | consumers of inspector output model three outcomes, not two | [`tools/acceptance.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/acceptance.py); **UNENFORCED** generally |
...
round-trips. [`tools/dlss.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/dlss.py) resynced `D:\Aowlspt\ConsistencyInfo` — the plain JSON
...
| G2 | the structural identity `P+5+ivLen+cipherLen == S-8` holds | [`tools/consistency.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/consistency.py) |
...
2. <code>R shared &lt;RVA&gt;</code>. `shared` → do not detour. `unknown` → **refuse**.
...
7. `VirtualQuery` / `aowl_is_readable` on **every** hop — <code>a-&gt;b-&gt;c</code> is three checks.
8. Exactly **one** `aowl_p_p_seh` around the whole body. It is **not re-entrant**:
   a nested inner guard *disarms the outer one*.
9. Every loop capped.
10. Flag-gated, **default OFF**, key name checked with [`tools/hostcfg.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostcfg.py).
...
17. `python tools/drainaudit.py`, [`tools/abilint.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/abilint.py), [`tools/allocasserts.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/allocasserts.py) on the
...
1. **X1 — a gated export called without its token.** Blast radius: the whole
   ...
   [`tools/gateaudit.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/gateaudit.py) — parse the 40 names out of
   [`abi/aowlspt_il2cpp_gates_data.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_il2cpp_gates_data.h), grep every call site in `host/` and `mods/`,
