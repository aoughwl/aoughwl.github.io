---
title: "Port plan \u2014 ammo loading animations"
---

# Port plan — Manimal Ammo Loading Animations → aowlspt (post-1.0 IL2CPP)

::: info From the aowlspt repository — [`docs/PORT_manimal_ammo_loading_animations.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/PORT_manimal_ammo_loading_animations.md)
...
::: warning A port plan
Read-only research into porting a pre-1.0 mod. Nothing was deployed.
:::
...
4. **Route** the item through that controller with four dispatch prefixes:
   - `Player.SetInHandsUsableItem` → <code>Proceed&lt;LoadAmmoBundleController&gt;</code> and skip.
   - `ClientUsableItemController.smethod_11` → <code>smethod&#95;7&lt;...&gt;</code> for our item.
...
5. **Drive** it: create item via `ItemFactoryClass.CreateItem`, `DropCurrentController`
   → <code>smethod&#95;1&lt;LoadAmmoBundleController&gt;</code> → <code>smethod&#95;8&lt;...&gt;</code> → `SpawnController`,
...
Measured with [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py) against the live `GameAssembly.dll` +
...
|---|---|---|---|
| <code>EFT.Player::SetInHandsUsableItem(Item, Callback&lt;IUsableItemController&gt;)</code> | `0x740DD0` | unique | dispatch entry |
...
| <code>EFT.Player::SetEmptyHands(Callback&lt;IEmptyHandsController&gt;)</code> | `0x740620` | unique | |
...
2. **Generic controller-swap methods instantiated with the injected type** —
   <code>Proceed&lt;LoadAmmoBundleController&gt;</code>, <code>smethod&#95;1&lt;...&gt;</code>, <code>smethod&#95;8&lt;...&gt;</code>. A
...
- **The AssetBundle itself is loadable.** [`mods/textures/redirect.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/textures/redirect.nim) already
...
- **But** [`mods/textures/redirect.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/textures/redirect.nim) also records a hard host limitation: the
  ...
  by mutating an argument (or by calling <code>Proceed&lt;T&gt;</code> in place of the original)
...
2. **Serve the custom item + bundle** from the backend (clone PortableRangeFinder,
   `UsePrefab.path`; redirect the new bundle key like [`mods/textures`](https://github.com/aoughwl/aowlspt/tree/main/mods/textures)). Verify the
