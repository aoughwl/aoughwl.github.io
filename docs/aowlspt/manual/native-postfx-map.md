---
title: "The post-processing map"
---

# NATIVE POSTFX MAP — driving the game's OWN rendering from a POSTFX subtab

::: info From the aowlspt repository — [`docs/NATIVE-POSTFX-MAP.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/NATIVE-POSTFX-MAP.md)
...
::: warning Map only — nothing implements it
This is offline reconnaissance, not a description of shipped behaviour. The native post-processing work was [shelved before 1.0](./shelved-native-settings).
:::
...
<code>GameSetting&lt;T&gt;</code> reference held by the group:
...
### 1.2 Do NOT go through <code>GameSetting&lt;T&gt;</code> — it is a shared generic
...
Fields for the walk (MEASURED): `instance` (static), <code>&lt;PostFX&gt;k&#95;&#95;BackingField @0x58</code> → GradingPostFX, <code>&lt;Camera&gt;k&#95;&#95;BackingField@0x70</code>, `_prismEffects@0xE8` →
...
live with <code>component &lt;camera&gt; PrismEffects</code>.
...
1. **Root.** `CameraManager::get_Instance()` @`0x1263BD0`, prologue-verified
   against the startup snapshot ([`abi/aowlspt_prologue.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_prologue.h)), called inside the
...
2. **Walk, validating every hop** (`aowl_is_readable` per dereference):
   ...
   <code>instance → &lt;PostFX&gt;k&#95;&#95;BackingField@0x58</code> for the colour block. Never an
   offset that can read null without a check — that is how `_rectTransform@0x78`
   burned a day.
3. **Write** through `hostfieldwrite` / [`abi/aowlspt_fieldrefs.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_fieldrefs.h): one
   ...
   [`tools/fieldrefs.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/fieldrefs.py)) refuses a wrong-klass receiver, a narrowing store or a
