---
title: "The backend, made fast"
---

# The backend, made fast

::: info From the aowlspt repository — [`docs/PERF-SERVER.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/PERF-SERVER.md)
...
Every number here was taken with [`tools/benchbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/benchbackend.nim) on this machine
(Windows 10, msys2 UCRT64 gcc, loopback), against the real emulator
([`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)) over the real wire protocol — sockets, HTTP, zlib framing, the
...
**Two backends, one mod.** Another agent was rewriting [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) while this
...
- The backend's own `toBytes`/`fromBytes` were <code>for i in 0 ..&lt; len: result.add</code>
...
- The emulator reads <code>templates.items.&lt;id&gt;.&#95;props.Width</code>, `.Height` and
...
[`aowl/src`](https://github.com/aoughwl/aowlspt/tree/main/aowl/src) and the ABI, which this work did not own — see the API gaps below.
...
[`backend/perfphase.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/perfphase.nim), off by default, one predictable branch per call site
...
The report is written to <code>&lt;root&gt;/perf.txt</code> once a second as well as at exit,
...
<code>&lt;root&gt;/store</code> — that is what the module is for — and neither can be told when
...
less its 73 ms of host calls — is the emulator's own work in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov),
...
[`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h), a header both hosts include, and it is the right next
...
- **Caching store values in memory.** It is the obvious next step and it is
  ...
  <code>&lt;root&gt;/store</code>, and a value one of them cached is a value the other can
...
- **Reusing one `z_stream` per thread instead of `compress2`.** `compress2` is
  ...
  small bodies. It was reverted, and [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h) is unchanged, because
...
a poll-driven accept and read path in [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h). This is that.
...
[`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h) along with what it costs: an O(connections) scan per
...
[`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim), [`backend/jsondb.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/jsondb.nim) and [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) while this
...
[`mods/tarkov/emu/profile.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/emu/profile.nim) says in as many words that this narrows and does
...
in [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h):
...
Later the same day the emulator stopped booting at all on [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)'s
...
- `aowl_net_serve`, `aowl_net_running` and `aowl_net_stop` keep their
  ...
  request. [`tools/aowlprobe.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowlprobe.nim), the only other file in the repo that defines
...
Fixing that is [`tools/firstrun.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/firstrun.nim)'s to make.
...
re-indexed on the next lookup (`gDirty` in [`backend/jsondb.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/jsondb.nim)).
...
| the `aowl test` floor | `--floor 800` | it is `--floor 500` in [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) |
...
decide. It was decided in [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim) afterwards, and the
...
endpoint, and the change that would is in [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim), which this
...
[`backend/dbrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/dbrace.nim) is extended for it, and the extension was checked the way
...
- **`items/moving`, the endpoint that matters.** 1.04 ms, 59 us of it in
  ...
  and argued in [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim).
- **`/client/items`.** 6.8 ms for 4.25 MB and 16.7 ms for 12.5 MB after change
  ...
  [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) and [`aowl/src`](https://github.com/aoughwl/aowlspt/tree/main/aowl/src). The client's own `inflate` is larger than all of
...
[`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim), which this work does not own*. This is that file.
...
[`tests/storeperf`](https://github.com/aoughwl/aowlspt/tree/main/tests/storeperf) prices the steps of the shape it has, timed inside C around
...
handle lives -- and *the two hosts running at once against one <code>&lt;root&gt;/store</code> is
...
[`tests/storeperf`](https://github.com/aoughwl/aowlspt/tree/main/tests/storeperf), same machine, 300 commits:
...
temporary is <code>&lt;dir&gt;/.hist/&lt;key&gt;.tmp</code>, which is per key and not per process, and
...
fix is a per-process suffix on the temporary -- <code>&lt;key&gt;.&lt;pid&gt;.tmp</code> -- which is
...
not made here because `tools/storecrash` asserts on the exact name <code>&lt;key&gt;.tmp</code>
