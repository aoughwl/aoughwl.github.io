---
title: "Debugging a native mod"
---

# Debugging a native mod

::: info From the aowlspt repository — [`docs/DEBUGGING.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/DEBUGGING.md)
...
`aowlspt-sim` loads a mod through the same [`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) the two
...
- `--store DIR` — where the mod's store lives. The default is
  <code>%TEMP%&#92;aowlspt-sim&#92;&lt;mod&gt;</code> and it **persists between runs**, so "save, exit,
...
in the mod's <code>nimcache/&lt;hash&gt;/</code> directory and the debug info points at it, so
...
[`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) straight into the mod's `aowlspt_init`, across the ABI
...
<code>&lt;store&gt;/shadow/</code>, because Windows locks a mapped DLL and the file you are
...
your install, see `aowlspt-verify` ([`tools/verifyinstall.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/verifyinstall.nim), built by `aowl
...
<code>&lt;root&gt;/mods</code>, so point `--root` at a scratch directory holding your build
...
the entire reason <code>Logs&#92;&lt;ts&gt;&#92;&#42; aiData&#95;000.log</code> holds one line and never the
interesting ones.
...
[`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim), so the wording below is the same in the backend, the
client host and the simulator:

| message | meaning |
|---|---|
| <code>could not load &lt;path&gt; (error 126)</code> | a *dependency* is missing, not the file itself |
...
| <code>&lt;name&gt;: describe failed</code> | `aowlspt_describe` returned non-OK |
| <code>&lt;name&gt; does not support the server side; skipping</code> | add the side to `sides = {...}` |
...
hook has fired. The boot log says the same thing. See [IL2CPP.md](./il2cpp).
