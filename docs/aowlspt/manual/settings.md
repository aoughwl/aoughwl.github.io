---
title: "In-game settings and the config schema"
---

# In-game settings (F12) and the mod config schema

::: info From the aowlspt repository — [`docs/SETTINGS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/SETTINGS.md)
...
Part 1 of this page argues for a hand-drawn overlay rather than driving Tarkov's own settings screen. That decision held: the native work was [shelved before 1.0](./shelved-native-settings), and the surface that shipped is the browser page and the [HTTP settings routes](./ui-api). Parts 2 and 3 — the config schema and the SPT config surface — are current.
:::
...
2. **That is the exact crash class this client has proven repeatedly.** Live
   ...
   thread crash this build (see [`abi/aowlspt_il2cppready.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_il2cppready.h): even *asking*
...
3. **`il2cpp_runtime_invoke` is on the wrong thread.** Driving the tab builders
   ...
   runs on the **host** thread ([`host/Aowlspt.Host.Il2Cpp`](https://github.com/aoughwl/aowlspt/tree/main/host/Aowlspt.Host.Il2Cpp)), and instantiating
...
> thread, not to avoid managed work. See [`abi/aowlspt_bridge.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_bridge.h) and the
> `il2cpp` bridge in [`host/Aowlspt.Host.Il2Cpp/aowlhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/aowlhost.nim).
>
...
>
> **The RVAs are trustworthy.** The offline resolver [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py)
...
[`abi/aowlspt_overlay.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_overlay.h) already draws a panel by hand inside a hooked
...
[`host/Aowlspt.Host.Il2Cpp/aowlhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/aowlhost.nim)). It is now the **built** settings
...
- `declaredSchemaJson(): Json` — the declared schema with **current values**
  ...
  body a <code>/aowlspt/settings/&lt;guid&gt;</code> route returns; the UI renders it directly.
...
implemented (see [`mods/fov/fov.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/fov/fov.nim) `fovSchema`), matching its module comment.

---

## 3. The SPT server config surface

[`reference/spt-config-settings.json`](https://github.com/aoughwl/aowlspt/blob/main/reference/spt-config-settings.json) (generated by [`tools/genspt_settings.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/genspt_settings.py)
from [`reference/spt-4.1-surface.json`](https://github.com/aoughwl/aowlspt/blob/main/reference/spt-4.1-surface.json)) is the full SPT 4.1 server config
...
- **All `implemented = false`.** aowlspt is a from-scratch emulator with no SPT
  ...
  its key to the `IMPLEMENTED` set in [`tools/genspt_settings.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/genspt_settings.py) and regenerate.
...
## 5. The built settings screen ([`abi/aowlspt_overlay.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_overlay.h))
...
- a **mod page** from <code>GET /aowlspt/settings/&lt;guid&gt;</code> — the mod's own declared
  ...
  **client-only** (`{sideClient, sideSim}`), as is [`mods/graphics`](https://github.com/aoughwl/aowlspt/tree/main/mods/graphics) (`{sideClient}`,
  twenty wired settings) and the client half of [`mods/admin`](https://github.com/aoughwl/aowlspt/tree/main/mods/admin): their routes are
  ...
  [`mods/settingshub`](https://github.com/aoughwl/aowlspt/tree/main/mods/settingshub)) could never hear them and their pages were absent from the
  F12 nav entirely.

  **This is now fixed by the in-process schema push.**
  [`host/Aowlspt.Host.Il2Cpp/settingsbridge.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/settingsbridge.nim) runs the same broadcast inside
  ...
  `SettingsPageAnnounce` (both added to [`aowl/src/aowlspt/settings.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/settings.nim)). It
  ...
  <code>GET /aowlspt/settings/&lt;guid&gt;</code> for it off a **fallback prefix** -- exact routes
  ...
  **the overlay needed no change at all**.
  ...
  [`tools/settingsdump.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/settingsdump.py) is the check: it fetches the index and every page and
...
- the **SPT pages** from the new server-side **settings-hub mod**
  ([`mods/settingshub`](https://github.com/aoughwl/aowlspt/tree/main/mods/settingshub)): `GET /aowlspt/settings/spt/index` for the page list and
  <code>GET /aowlspt/settings/spt/page/&lt;id&gt;</code> for one page. The hub carries the whole
  SPT surface embedded in source ([`mods/settingshub/sptsurface.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/settingshub/sptsurface.nim), generated by
  [`tools/gen_sptsurface.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/gen_sptsurface.py) from [`reference/spt-config-settings.json`](https://github.com/aoughwl/aowlspt/blob/main/reference/spt-config-settings.json)), because
  nimony has no compile-time file read.

**Writing an edit back.** A change on an implemented mod row POSTs
<code>&#123;"key":..,"value":&lt;literal&gt;&#125;</code> to that mod's <code>/aowlspt/settings/&lt;guid&gt;</code> route;
...
<code>SettingsTab::CreateControl&lt;T&gt;</code> @0x2B86E20 — one shared-generic body serving
...
<code>/aowlspt/settings/&lt;guid&gt;</code>. Phase 3 does not replace it; it adds a *second
...
- **Fetch a mod's schema in-game/offline:** GET <code>/aowlspt/settings/&lt;guid&gt;</code> —
