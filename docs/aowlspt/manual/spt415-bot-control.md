---
title: "SPT 4.1.5 bot control"
---

# SPT 4.1.5 bot control -- the measured map and the design

::: info From the aowlspt repository — [`docs/SPT415-BOT-CONTROL.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/SPT415-BOT-CONTROL.md)
...
::: warning A different client
This page targets **stock, pre-1.0 SPT 4.1.5** — a Mono client with BepInEx and the original SAIN — not the post-1.0 IL2CPP client the rest of aowlspt is about. It was a parallel study of the same backend driving a second target, and nothing in it ran against a live SPT install.
:::

**A separate study: driving bot spawns in a stock, pre-1.0 SPT 4.1.5 install from the same backend. Different client, different runtime.**
...
2026-09-07 with Mono.Cecil ([`mods/basement/spt/MemberCheck`](https://github.com/aoughwl/aowlspt/tree/main/mods/basement/spt/MemberCheck), plus a scratch
...
| [`mods/basement/spt/Basement.Server`](https://github.com/aoughwl/aowlspt/tree/main/mods/basement/spt/Basement.Server) (SPT server mod, C#) | BUILT: `dotnet build` 0 errors 0 warnings; 4 Harmony patches + status route; NOT installed, NOT run |
...
* <code>GenerateBotsRequestData &#123; List&lt;GenerateCondition&gt; Conditions &#125;</code>,
...
* <code>BotGenerationDetails &#123; bool IsPmc; string Role, RoleLowercase, Side; int? PlayerLevel; string PlayerName; MinMax&lt;int&gt; LocationSpecificPmcLevelOverride; int BotRelativeLevelDeltaMax/Min; int BotCountToGenerate; string BotDifficulty; bool IsPlayerScav; string EventRole; bool AllPmcsHaveSameNameAsPlayer; string Location; bool ClearBotContainerCacheAfterGeneration; int BotLevel; string GameVersion &#125;</code>.
...
* The override mechanism SPT ships for servers is **Harmony through
  ...
  <code>IEnumerable&lt;IRuntimePatch&gt;</code> by ctor injection and calls `Enable()` on each.
...
* Custom routes: subclass <code>StaticRouter(JsonUtil, IEnumerable&lt;RouteAction&gt;)</code>
  with `[Injectable]`; <code>RouteAction&lt;T&gt;(string url, Func&lt;string, T, MongoId, string, CancellationToken, ValueTask&lt;string&gt;&gt;)</code>;
  body helpers <code>HttpResponseUtil.GetBody&lt;T&gt;(T, BackendErrorCodes, string, bool)</code> / <code>NoBody&lt;T&gt;(T)</code>;
  <code>JsonUtil.Serialize&lt;T&gt;(T, bool indented)</code> / <code>Deserialize&lt;T&gt;(string)</code>.
  `EmptyRequestData` is `SPTarkov.Server.Core.Models.Eft.Common.EmptyRequestData`.
* Mod folder: `ModHelper.GetAbsolutePathToModFolder(Assembly)`, <code>GetJsonDataFromFile&lt;T&gt;(path, file)</code>.
* Logging: <code>SPTarkov.Common.Models.Logging.ISptLogger&lt;T&gt; &#123; Success/Error/Warning/Info/Debug/Critical(string, Exception) &#125;</code>.

Reference mod layout on disk (MEASURED): <code>SPT&#95;Runtime&#92;user&#92;mods&#92;&lt;Name&gt;&#92;&lt;Name&gt;.dll</code> + data files; no manifest, the `IModMetadata` record is the manifest.
...
* `POST /client/match/local/start` (ldstr in `MatchStaticRouter..ctor`) ->
  ...
  <code>StartLocalRaidResponseData.set&#95;ExcludedBosses(List&lt;string&gt;)</code>.
...
* `GenerateLocationAndLoot` body: `LocationTable` -> `Location.Base` ->
  <code>ICloner.Clone&lt;LocationBase&gt;</code> -> `PmcWaveGenerator` -> `ProfileActivityService.GetProfileActivityRaidData(sessionId).RaidAdjustments`
...
* `LocationBase` (MEASURED props that matter): <code>string Id, Name; List&lt;Wave&gt; Waves; List&lt;BossLocationSpawn&gt; BossLocationSpawn; IEnumerable&lt;SpawnPointParam&gt; SpawnPointParams; string OpenZones; NonWaveGroupScenario NonWaveGroupScenario; int BotMax, BotStart; int? BotStop, BotSpawnCountStep, BotSpawnPeriodCheck, BotSpawnTimeOnMin/Max, BotSpawnTimeOffMin/Max, MaxBotPerZone, BotMaxPvE, ...</code>.
* <code>Wave &#123; string BotPreset, BotSide, SpawnPoints /&#42;zone name&#42;/, SptId, OpenZones; bool? KeepZoneOnSpawn, IsPlayers; WildSpawnType? WildSpawnType; int? Number, SlotsMax, SlotsMin, TimeMax, TimeMin, ChanceGroup; HashSet&lt;?&gt; SpawnMode &#125;</code>.
...
* `EFT.BotsController` (public): `Init(IBotGame, IBotCreator, BotZone[], ISpawnSystem, BotLocationModifier, bool botEnable, bool freeForAll, bool enableWaveControl, bool online, bool haveSectants, IPlayersCollection, string openZones, BotLocationEvents)`;
  `BotSpawner BotSpawner {get}`; `BotsList Bots` (field; <code>IEnumerable&lt;BotOwner&gt; BotOwners</code>, `Count`, `OnBotAdd/OnBotRemove`);
  ...
  Reached through <code>Comfort.Common.Singleton&lt;IBotGame&gt;.Instance.BotsController</code> (`IBotGame { BotsController, BossSpawnScenario, GameDateTime, Status, WeatherCurve }`; <code>Singleton&lt;T&gt; &#123; static T Instance; static bool Instantiated &#125;</code>) -- the path MoreBotsAPI's `HuntManager.InitRaid` uses (`Singleton`1.get_Instance -> IBotGame.get_BotsController -> BotsController.get_BotSpawner -> BotSpawner.add_OnBotCreated`).
...
* Names: <code>BotNameService.GenerateUniqueBotNickname(BotType, BotGenerationDetails, HashSet&lt;string&gt; uniqueRoles)</code>, `AddRandomPmcNameToBotMainProfileNicknameProperty(BotBase)`, `ClearNameCache()` (called at raid start). The bot's name ends up in `Info.Nickname`, `Info.LowerNickname` (and `MainProfileNickname` for PMC chat).
...
* `SAIN.Interop.SAINExternal` (static): `IgnoreHearing(BotOwner, bool, bool ignoreUnderFire, float duration)`, `GetPersonality(BotOwner) : string`,
  `ExtractBot(BotOwner)`, `GetExtractedBots(List)`, <code>GetExtractionInfos(List&lt;ExtractionInfo&gt;)</code>, `TrySetExfilForBot(BotOwner)`, `TimeSinceSenseEnemy(BotOwner)`,
...
* `SAIN.Components.BotComponent : BotComponentBase : MonoBehaviour` -- obtained by `BotManagerComponent.Instance` then <code>Component.GetComponent&lt;BotComponent&gt;()</code> (body of `SAINExternal.GetBotComponent`).
...
* SAIN is a set of **BigBrain layers**: `SAIN.Layers.SAINLayer : DrakiaXYZ.BigBrain.Brains.CustomLayer`; `CombatSoloLayer ("Combat Layer")`, `CombatSquadLayer ("Squad Layer")`, `ExtractLayer`, `SAINAvoidThreatLayer`, `SAINFlashedLayer`, `DebugLayer`;
  ...
  <code>BrainManager.AddCustomLayer(Type, List&lt;string&gt; brainNames, int priority&#91;, List&lt;WildSpawnType&gt; roles&#93;)</code>.
...
* BigBrain 1.5.0 API (`DrakiaXYZ.BigBrain.Brains`): <code>BrainManager.AddCustomLayer(Type, List&lt;string&gt;, int priority)</code>, `RemoveLayer(s)`, `RestoreLayer(s)`, `IsCustomLayerActive(BotOwner)`, `GetActiveLayerName(BotOwner)`, `GetActiveLayer/Logic(BotOwner)`;
  ...
  `CustomLayer.Action(Type logicType, string reason, ActionData data)`, <code>abstract class CustomLogic(BotOwner) : CustomLogic&lt;CustomLayer.ActionData&gt; &#123; Start(); Stop(); Update(ActionData) &#125;</code>.
...
* The vanilla members the aowlspt SAIN port already names ([`mods/sain/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/README.md)) exist unchanged on 4.1.5: `BotOwner.Mover : BotMover`, `BotMover.GoToPoint(Vector3 pos, bool slowAtTheEnd, float reachDist, bool getUpWithCheck, bool mustHaveWay, bool onlyShortTrie, bool force) : NavMeshPathStatus`,
  ...
  <code>BotOwner.BotsGroup : BotsGroup &#123; bool AddEnemy(IPlayer, EBotEnemyCause); RemoveEnemy(IPlayer, EBotEnemyCause); AddNeutral(IPlayer); AddAlly(Player); IsEnemy/IsAlly(IPlayer); Member(int); MembersCount; List&lt;IPlayer&gt; Allies; AddMember(BotOwner) &#125;</code>,
...
(i) **Server mod [`mods/basement/spt/Basement.Server`](https://github.com/aoughwl/aowlspt/tree/main/mods/basement/spt/Basement.Server)** (built). Four `AbstractPatch`es, each fail-open and loud (a sidecar miss = SPT's own output, one warning line):
...
| `LocationWavesPatch` | postfix `LocationLifecycleService.GenerateLocationAndLoot` | <code>GET /aowlspt/basement/spt/waves?map=&lt;LocationBase.Id&gt;&amp;raidId=</code>; `{ok, clear, waves:[EFT wave rows], bossWaves:[EFT BossLocationSpawn rows]}` parsed by SPT's `JsonUtil` into SPT's own records; `clear` empties the stock lists first; a wave whose `SpawnPoints`/`BossZone` is not a `BotZoneName` in `SpawnPointParams` is DROPPED with a warning, never guessed |
...
* `GroupSpawn.cs` -- `group.spawn {factionId, count, near{x,y,z}, radiusM, people[], role?, mode?}`:
  ...
  `await BotCreationData.Create(pd, creator, count, spawner)`, then <code>SpawnBotsInZoneOnPositions(List&lt;ISpawnPoint&gt;&#123;our ScenePoint rows on a ring of radiusM&#125;, zone, data, cb)</code>.
...
2. `GET :6969/aowlspt/basement/spt/status` -> `sidecarFailures == 0` and, after one raid start, <code>plan.waves&#91;session&#93; &gt;= 1</code> and <code>named &gt; 0</code>. `named == 0` with <code>planned &gt; 0</code> = FAIL (the postfix did not fire or the roles did not match).
...
## 4. Backend patch block (for the coordinator; [`mods/basement/basement.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/basement/basement.nim) is held by others)
...
are [`aowl/src/aowlspt/json.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/json.nim) (no `items` iterator exists, hence the index
loops); `obj/arr/put/add/done/errJson` are [`aowl/src/aowlspt/server.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/server.nim)
...
6. Harmony on an <code>async Task&lt;&gt;</code> method (`BotController.Generate`): a PREFIX on
...
* `MemberCheck` could not answer three questions this task needed, so a scratch
  copy (`mc2`) gained verbs: <code>virt &lt;Type&gt;</code> (virtual/override/sealed/visibility per
  method -- the fact that NOTHING in SPT's bot pipeline is virtual decided the
  whole server design), <code>attrs &lt;Type&gt;</code> (custom attributes with ctor args and
  ...
  <code>impl &lt;substr&gt;</code> (types implementing/extending), `strings [substr]` (ldstr per
  method -- how route URLs and brain names were found), <code>publics &lt;substr&gt;</code>,
  <code>msig &lt;Type&gt; &lt;Member&gt;</code> (FULL parameter type names; `sig` prints `List`1`),
  `ints &lt;Type::Method>` (ldc.i4 operands + calls in order -- how SAIN's layer
  priorities were read). Recommend folding all eight into
  [`mods/basement/spt/MemberCheck/Program.cs`](https://github.com/aoughwl/aowlspt/blob/main/mods/basement/spt/MemberCheck/Program.cs); they are ~40 lines.
