---
title: "The settings UI API"
---

# The aowlspt settings UI API

::: info From the aowlspt repository — [`docs/UI-API.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/UI-API.md)
...
fallback browser page at `/aowlspt/ui/page/settings` ([`mods/uihub`](https://github.com/aoughwl/aowlspt/tree/main/mods/uihub)), and any
...
setting, via [`aowl/src/aowlspt/settings.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/settings.nim). If a UI needs something a route
...
| GET | <code>/aowlspt/settings/&lt;guid&gt;</code> | `[{Setting}...]` — that mod's schema, current values folded in |
| POST | <code>/aowlspt/settings/&lt;guid&gt;</code> | body <code>&#123;"key":"&lt;configKey&gt;","value":&lt;json literal&gt;&#125;</code>; persists the edit — creating the key in `config.json` if it was never there. Reply is `[{Setting}...]` on success; on failure it is instead <code>&#123;"err":"&lt;reason&gt;","rows":&#91;&#123;Setting&#125;...&#93;&#125;</code> — an OBJECT, not an array, deliberately, so `Array.isArray(reply)` is the one-line check for "did this actually persist" |
| POST | <code>/aowlspt/settings/&lt;guid&gt;/reset</code> | body <code>&#123;"key":"&lt;configKey&gt;"&#125;</code> resets that one key, or an empty body `{}` resets every key this mod declares — in both cases to the value in that key's `default` field, the schema's one source of truth. Same success/failure reply shape as the write route above |
| GET | `/aowlspt/settings/spt/index` | `{"pages":[{id,label,count,done}...]}` — the read-only SPT config catalogue ([`mods/settingshub`](https://github.com/aoughwl/aowlspt/tree/main/mods/settingshub)) |
| GET | <code>/aowlspt/settings/spt/page/&lt;id&gt;</code> | `{"settings":[...],"nested":[...]}` — one SPT config page, all `implemented:false` today |
| GET | `/aowlspt/ui/page/settings` | the settings HTML page ([`mods/uihub`](https://github.com/aoughwl/aowlspt/tree/main/mods/uihub)), for a human with a browser |
| GET | `/aowlspt/ui/lib/settings.js` | the settings CLIENT LIBRARY the page above is built out of, as `window.AowlSettings` — served `text/javascript`, uncompressed, so any page can <code>&lt;script src&gt;</code> it |
| GET | <code>&lt;a row's optionsUrl&gt;?q=&lt;term&gt;&amp;limit=&lt;n&gt;</code> | OPTIONAL, owned by the declaring mod: `{"options":[{"value","label"}...],"matched":N}` — how a `type:"select"` row with thousands of choices is searched without shipping them in the schema |
...
One row of <code>GET /aowlspt/settings/&lt;guid&gt;</code>:
...
instruction to the renderer: **do not build a <code>&lt;select&gt;</code>, build a searchable
...
means "the first n, unfiltered". [`tests/uihubscale`](https://github.com/aoughwl/aowlspt/tree/main/tests/uihubscale) is the reference
...
[`backend/aowlbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/aowlbackend.nim)). The native client and the overlay's own HTTP
...
the JSON returned, only the bytes on the wire. [`mods/uihub`](https://github.com/aoughwl/aowlspt/tree/main/mods/uihub)'s page does this
...
[`mods/uihub`](https://github.com/aoughwl/aowlspt/tree/main/mods/uihub) keeps NO private copy of any of this. Its page is a shell around
...
hash (<code>#guid=&lt;guid&gt;&amp;cat=&lt;category&gt;&amp;q=&lt;query&gt;</code>), incremental rendering so
...
Nothing here is special to [`mods/uihub`](https://github.com/aoughwl/aowlspt/tree/main/mods/uihub) — it is the same two GET shapes and
