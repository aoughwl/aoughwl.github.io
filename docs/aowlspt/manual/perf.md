---
title: "What a client-side call costs"
---

# What a client-side call costs

::: info From the aowlspt repository — [`docs/PERF.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/PERF.md)
...
[`tools/perfbench.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/perfbench.nim), against [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp), on an Intel i9-10850K,
...
Two paths the table above predates, measured by [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) against the
...
compiled <code>Single -&gt; Single</code> in the stand-in, reached through a bound call so
...
[`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) row above and 37 ns here -- and the gap is real rather than
...
**Allocation-free is measured, not asserted.** [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) fires a
...
[`abi/aowlspt_fast.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_fast.h) has the mechanism in detail; the short version:
...
- **Virtual dispatch, which the fast path does not do.** A bound call goes
  ...
  to reach a generic instantiation like <code>List&lt;Player&gt;</code>: it has no name
...
**It is a stand-in runtime.** [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp) implements the same C API over
...
maintains under <code>MI&#95;STAT &gt; 1</code> — a debug build — and reported a confident zero
...
allocates. `hostPatchStat` in [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) goes through `call`, which is
...
[`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) was free to edit. The timings are printed and only the three
