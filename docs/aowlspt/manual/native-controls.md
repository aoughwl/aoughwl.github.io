...
::: info From the aowlspt repository — [`docs/NATIVE-CONTROLS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/NATIVE-CONTROLS.md)
...
`.cache/global-metadata.dec.dat` with [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py) and a helper that reuses
...
<code>SettingsTab.CreateControl&lt;T&gt;(T prefab, Transform parent)</code> is the game's own one-liner. It
...
<code>SettingControl &lt;- EFT.UI.UIInputNode &lt;- EFT.InputSystem.InputNode &lt;- InputNodeAbstract &lt;- SerializedMonoBehaviour &lt;- MonoBehaviour</code>
...
<code>SettingToggle &lt;- SettingControl</code>. One own field: `0xa8 Toggle : UpdatableToggle`.

| RVA | sharedness | signature |
|---|---|---|
| 0x16fd0d0 | unique | <code>SettingToggle BindTo(GameSetting&lt;bool&gt; setting)</code> |
...
| RVA | sharedness | signature |
|---|---|---|
| 0x16fb4c0 | unique | <code>SettingFloatSlider BindTo(GameSetting&lt;int&gt; setting, int minValue, int maxValue)</code> |
| 0x16fb810 | unique | <code>SettingFloatSlider BindTo(GameSetting&lt;float&gt; setting, float minValue, float maxValue, string format)</code> |
...
| 0x16fb400 | unique | <code>ReadOnlyCollection&lt;int&gt; GetIndexCollection(ICollection source)</code> |
| — | no-code | `BindTo(...)`, `BindToEnum(...)`, `BindDropDownToSetting(...)`, `UpdateDropDownValue(...)` — all generic. GAP. |

### The inner widgets the rows wrap

<code>EFT.UI.NumberSlider &lt;- UIInputNode</code> — `_slider : UnityEngine.UI.Slider @0x80`,
`_valueInput : TMP_InputField @0x88`, `_format @0x90`, `_maxValue @0x98`, `_minValue @0x9c`,
<code>&#95;onValueChanged : Action&lt;float&gt; @0xa8</code>.

| RVA | sharedness | signature |
|---|---|---|
| 0x16b4ea0 | unique | `void Show(float minValue, float maxValue, string format)` |
| 0x16b5300 | unique | `void SetCurrentValue(float value)` |
| 0x16b5710 | unique | <code>void UpdateValue(float value, bool sendCallback, Nullable&lt;float&gt; min, Nullable&lt;float&gt; max)</code> |
| 0x6910c0 | **shared x34** | <code>void Bind(Action&lt;float&gt; valueChanged)</code> |
| 0x16b5850 | unique | `float CurrentValue()` |

<code>EFT.UI.SelectSlider &lt;- UIElement</code> — `_slider @0x70`, `_valueText : TextMeshProUGUI @0x78`,
`_notchTemplate : GameObject @0x80`, `_notchContainer : RectTransform @0x88`,
<code>&#95;notches : List&lt;GameObject&gt; @0x90</code>, <code>&#95;onValueChanged : Action&lt;int&gt; @0x98</code>, `_values : string[] @0xb0`.

| RVA | sharedness | signature |
|---|---|---|
| 0x16b9e30 | unique | `void Show(string[] values)` |
| 0x16ba060 | unique | <code>void Show(Func&lt;string&#91;&#93;&gt; values)</code> |
| 0x16ba340 | unique | <code>void UpdateValue(int value, bool sendCallback, Nullable&lt;int&gt; min, Nullable&lt;int&gt; max)</code> |
| 0x16ba670 | unique | `void SetLabelText(string text)` |
| 0x691000 | **shared x38** | <code>void Bind(Action&lt;int&gt; valueChanged)</code> |

<code>EFT.UI.DropDownBox &lt;- BaseDropDownBox &lt;- InteractableElement &lt;- UIElement</code> —
`_currentValueText @0xf0`, `_button : UnityEngine.UI.Button @0xf8`, `_background : Image @0x100`.

| RVA | sharedness | signature |
|---|---|---|
| 0x16afc80 | unique | <code>void Show(IEnumerable&lt;string&gt; values, Func&lt;int,bool&gt; validator)</code> |
...
<code>EFT.UI.UpdatableToggle &lt;- UnityEngine.UI.Toggle &lt;- Selectable &lt;- UIBehaviour</code>.
...
`UpdatableToggle` adds <code>&#95;onValueChanged : Action&lt;bool&gt; @0x128</code>.
...
| 0x16c4ba0 | unique | <code>UpdatableToggle::void UpdateValue(bool value, bool sendCallback, Nullable&lt;bool&gt; min, Nullable&lt;bool&gt; max)</code> |
| 0x691790 | **shared x16** | <code>UpdatableToggle::void Bind(Action&lt;bool&gt; valueChanged)</code> |
...
<code>EFT.UI.UIAnimatedToggleSpawner &lt;- EFT.UI.UISpawner&#96;1&lt;AnimatedToggle&gt; &lt;- EFT.UI.UIElement</code>.
...
<code>EFT.UI.UISpawnableToggle &lt;- ButtonFeedback &lt;- InteractableElement &lt;- UIElement</code> —
...
5. Each `ShowXxx` calls **<code>SettingsTab.CreateControl&lt;T&gt;(T prefab, Transform parent)</code>**
   (generic, no-code), which instantiates the prefab under the parent and appends it to
   <code>SettingsTab.&#95;createdControls : List&lt;SettingControl&gt; @0x88</code>.
...
| offset | field | type |
|---|---|---|
| 0x80 | `OnLoadingInProgress` | <code>Action&lt;bool&gt;</code> |
| 0x88 | `_createdControls` | <code>List&lt;SettingControl&gt;</code> |
| 0x90 | <code>&lt;IsInitialized&gt;k&#95;&#95;BackingField</code> | `bool` |
...
| — | no-code | <code>T CreateControl&lt;T&gt;(T prefab, Transform parent)</code> — GAP, §5 |
...
| 0x138 | `_initializedTabs` | <code>HashSet&lt;ESettingsGroup&gt;</code> |
| `0x0` *(static)* | `_tabs` | <code>Dictionary&lt;ESettingsGroup,SettingsGroupObjects&gt;</code> — static; `0x0` here is a static-storage index, not an instance offset |
...
* **<code>SettingsTab.CreateControl&lt;T&gt;(T prefab, Transform parent)</code> has no RVA.** It is a
  ...
  returns `no-code`. **GAP** — and [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py) has no verb for the generic
...
* Same for every generic bind: `SettingDropDown.BindTo` / `BindToEnum` /
  ...
  The two **non-generic** binds — <code>SettingToggle.BindTo(GameSetting&lt;bool&gt;)</code> @0x16fd0d0 and
...
* **``Bsg.GameSettings.GameSetting`1`` and ``BaseSettingsController`1`` have NO layout.**
  ...
  <code>GameSetting&lt;bool&gt;</code> or <code>BaseSettingsController&lt;T&gt;</code> would be fabricated.** Take that layout
...
| a <code>GameSetting&lt;bool&gt;&#42;</code> to bind | **borrow an existing one** from a settings group, e.g. `GameSettingsTab @0xe8` (`GameSettingsGroup`) and the group's <code>GameSetting&lt;bool&gt;</code> fields listed in `docs/UI-MAP.md`. Constructing a fresh one is a GAP (§5). |
...
`Instantiate(Object,Vector3,Quaternion,Transform)` @0x52ad8c0. The **generic** <code>Instantiate&lt;T&gt;</code>
...
<code>Bind(Action&lt;float&gt;)</code> @0x6910c0 (**shared x34 — safe to CALL, never detour**),
...
* <code>findtext "&lt;my caption&gt;"</code> under the settings screen returns **exactly one ACTIVE** hit,
...
**STATUS 2026-09-01 (tooling agent): 1, 2, 3 and 5 are FIXED; 4 is not.** `il2cpp_resolve.py` gained <code>genericmethods &lt;Type&gt;&#91;::&lt;Name&gt;&#93;</code> (gated on a new `verify-generics` self-check with a negative control) and <code>typemethods &lt;Type&gt;&#91;--inherited&#93;</code>; `parent_type` now resolves 0x15 GENERICINST *and* 0x1c OBJECT bases (it was dropping 13,088 of 29,835 real base classes, not only the generic ones) and is gated by `verify-parents`; `gen_uimap.py` GROUPS now covers `EFT.UI.Settings.`, `EFT.UI.Toggle` and `UISpawner`, the INTERESTING name filter is gone, and the map is regenerated. <code>SettingDropDown::BindTo&lt;T&gt;</code> has 7 instantiated bodies; `SettingToggle::BindTo` is 0x16fd0d0 (unique). Item 4 (dict-vs-tuple field APIs) was left alone.


1. **`il2cpp_resolve.py` has no verb for generic-method instantiations.** Every
   <code>CreateControl&lt;T&gt;</code> / <code>BindTo&lt;T&gt;</code> / <code>GameSetting&lt;T&gt;</code> member resolves to `no-code`, so the
   ...
   `Il2CppCodeRegistration.genericMethodPointers`; a <code>genericmethods &lt;Type&gt;::&lt;Name&gt;</code> verb
...
2. **`Resolver.parent_type` returns `None` for a generic-instance base class.** It only
   ...
   <code>UIAnimatedToggleSpawner &lt;- UISpawner&#96;1&lt;AnimatedToggle&gt;</code> reads as "no base class". I had
...
3. **<code>il2cpp&#95;resolve.py methods &lt;substr&gt;</code> matches the METHOD name only**, so
   `methods EFT.UI.Settings.SettingToggle` returns a zero-hit "searched EXHAUSTIVELY"
   verdict that reads as "this type has no methods". A <code>typemethods &lt;Type&gt;</code> verb (what my
...
5. **[`tools/gen_uimap.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/gen_uimap.py) was run only with `--only EFT.Settings.`**, so `docs/UI-MAP.md`
