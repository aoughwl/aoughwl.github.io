---
title: "Actuating the local player"
---

# In-raid actuation of the local player (`pact`)

::: info From the aowlspt repository — [`docs/PLAYER_ACTUATION.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/PLAYER_ACTUATION.md)
...
| file | what is in it |
|---|---|
| [`abi/aowlspt_pact.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_pact.h) | the 31-row byte-verified call table, the derivation of every calling shape from the compiled bodies, the measured field offsets, and every refusal with its reason |
| [`host/Aowlspt.Host.Il2Cpp/pact.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/pact.nim) | the guard, the flag, the fault budget, the queue, the readbacks and the counters |
...
[`mods/sain/client/drivecalls.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/client/drivecalls.nim) drives bots through seven RVAs. They split
cleanly in two:
...
[`mods/ammoloading`](https://github.com/aoughwl/aowlspt/tree/main/mods/ammoloading) hooks.
...
1. capture; refuse if <code>n0 &lt;= 0</code> (nothing to unload) — counted **NEVER ISSUED**
...
5. poll `Magazine.get_Count()` until <code>&gt;= n0</code>, same deadline

Both inventory calls return <code>Task&lt;IResult&gt;</code> and are **asynchronous**. The task is
...
| <code>EFT.Player::Proceed(Weapon, Callback&lt;IFirearmHandsController&gt;, bool)</code> @`0x742710` and its ten siblings | same `Callback` problem. **Use `ECommand` 41 / 42 / 43** for weapon swap. |
...
<code>EftBattleUIScreenController.&lt;Owner&gt;k&#95;&#95;BackingField</code>, none of which we hold. A
...
Every hop is `VirtualQuery`-guarded separately (<code>a-&gt;b-&gt;c</code> is three checks). The
