...
::: info From the aowlspt repository — [`docs/MODDING.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/MODDING.md)
...
route — read [`mods/admintrader/`](https://github.com/aoughwl/aowlspt/tree/main/mods/admintrader) first. It is a complete, commented, working
...
server-side mechanism is small: [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) serves traders and quests straight
out of the loaded database, so a mod adds one by writing JSON at
<code>traders.&lt;id&gt;</code> / <code>templates.quests.&lt;id&gt;</code> with `dbWrite`. There is no trader API
...
**[`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) is the worked version of everything on this page**, in this
...
facility it cannot exercise and why, and against [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp) in a stage of
its own. That is the sample to copy from.

[`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) is **not**. It is the client host's regression test:
...
find out what the host is asserted to do. [`examples/clientprobe`](https://github.com/aoughwl/aowlspt/tree/main/examples/clientprobe) is the same mod
...
[`examples/highlevel/highlevel.nim`](https://github.com/aoughwl/aowlspt/blob/main/examples/highlevel/highlevel.nim)'s own header, which still says twenty-three
assertions and seventeen. Derived afresh 2026-08-19 by matching every
`find(l, "...")` in [`tools/hostharness.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostharness.nim) against what each mod actually
...
rather than crashing. [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) calls one method with a known answer
...
call as well as the capability. Every arm site in [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) does.
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) IS the game server, and it has no trader type and no quest type.
It reads <code>traders.&lt;id&gt;</code> and <code>templates.quests.&lt;id&gt;</code> straight out of the loaded
...
The full worked example is [`mods/admintrader`](https://github.com/aoughwl/aowlspt/tree/main/mods/admintrader) — 199 lines for a trader stocking
4,288 items plus a quest, with a status route and a settings page. Its
[`tools/acceptance_admintrader.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/acceptance_admintrader.py) asserts the result against a real `db.json`.
...
* **A locale name as a dotted path.** The text lives in `locales.global.en`
  under <code>"&lt;id&gt; Nickname"</code>, with a SPACE, so a dotted path writes a key nobody
...
[`tools/acceptance_admintrader.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/acceptance_admintrader.py)'s `check_shape` does; it accepts a field if it
...
a mod to load at all (`docs/MOD-ENABLE-PATH.md` — the DLL, [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json),
and the manager-owned selection, enabled via
<code>GET /aowlspt/mods/enable/&lt;guid&gt;</code>), and `regcheck` comparing `sides` and
...
[`examples/clientprobe`](https://github.com/aoughwl/aowlspt/tree/main/examples/clientprobe), and it is the escape hatch you drop to if you are
...
to replace, for reasons [PERF.md](./perf) measures. It exists because a great
...
replaces (<code>hostharness.exe &lt;stage&gt; --runtime tests&#92;mockil2cpp&#92;GameAssembly.dll --seconds 8</code>, the line `an address out of a handle costs`; re-run 2026-08-19,
where this page had 34 and 1146). The absolute numbers
move between runs; the two orders of magnitude between them do not.
[`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) re-measures both, on one object in one hook firing, in its own
...
into a trampoline — see [IL2CPP.md](./il2cpp). A function shorter than the
...
There are **256** patch slots (`AOWL_MAX_HOOKS`, [`abi/aowlspt_detour.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_detour.h)), and on
...
arms second. That is why [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) stages itself in its own directory
rather than into `installer\build\hoststage`, where [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) is
...
paragraph, which was derived from them. [ARCHITECTURE.md](./architecture)'s
...
postfix refusals are unchanged. [PERF.md](./perf) has the full table and
[ABI.md](./abi) the mechanism.
...
out loud. [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) makes it on purpose and prints what comes back:
...
assuming; the mechanism is in [IL2CPP.md](./il2cpp), and it has never been run
...
Args <code>&#123;"id":"&lt;your token&gt;","method":"GET&#124;POST","url":"http://127.0.0.1:6969/...", "body":"...","timeoutMs":25000&#125;</code>. Answers, **immediately**, either
...
Args <code>&#123;"path":"&lt;absolute path&gt;.wav","volume":0.0-1.0&#125;</code>. Answers `{"ok":true,...}`
or `{"ok":false,"why":".."}`.
...
groundwork is in [VOICE_RVA.md](./voice-rva).
...
[`abi/aowlspt_hostnet.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_hostnet.h) + [`abi/aowlspt_playwav.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_playwav.h) into
[`tests/hostnet/hostnet_drive.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/hostnet/hostnet_drive.c) and drives them against a real loopback HTTP
...
One file per key, held by the host under <code>store/&lt;your-guid&gt;/</code>. Use it for what
...
`store_*` out of the same [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim) the backend does, rooted
in a scratch directory under <code>%TEMP%&#92;aowlspt-sim&#92;&lt;mod&gt;</code> (`--store DIR` moves
it). So a mod whose whole job needs persistence — [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) — can be run
...
[MODMANAGER.md](./modmanager), and note that "synchronously" is exactly why a
...
want to prove that of your own mod; [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) does exactly that.
...
[PERF.md](./perf), "What the host costs per frame" — read it there rather than
...
thread" becomes a claim instead of a hope. [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) prints both ids in
...
**It needs no ABI call and no host support.** [`abi/aowlspt_lock.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_lock.h) is a
...
[`aowl/src/aowlspt/fast.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/fast.nim):
...
4. **Check one call against a value you can predict before trusting the
   ...
   read out of the wrong one, answers a plausible number. [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) calls
...
[`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp), `EFT.Player::SpawnCount` (static `Int32`) and
...
[`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) catches it at 100.0 and prints `1120403456` — with `ok` true,
...
[`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) times both side by side (both 1 ns on this machine) and prints
...
`classifyType(...) == fkNone` gate that `mods/classicmovement` and [`mods/fov`](https://github.com/aoughwl/aowlspt/tree/main/mods/fov) use
...
instantiation** such as <code>List&lt;Player&gt;</code>: it has no name `findClass` accepts, but
...
and do not make the claim. [`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) prints
...
[`examples/lesson`](https://github.com/aoughwl/aowlspt/tree/main/examples/lesson) is the guided one: it binds a static method, an object-bound
...
property getter where a field is a property on some builds. [`mods/fov`](https://github.com/aoughwl/aowlspt/tree/main/mods/fov) is a
...
[PERF.md](./perf). Read it before porting anything for speed: a naive native
...
builds a stand-in IL2CPP runtime ([`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp)) that exports the same C
API as `GameAssembly.dll`, loads the host against it, and runs
[`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) through resolve, call-with-arguments, live objects, fields
and patching:
...
`checkLog` in [`tools/hostharness.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/hostharness.nim), and the mod whose log it greps is
[`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) — fifteen of the twenty-four are satisfied only by lines
that file prints, and three more by lines it shares with [`examples/clientprobe`](https://github.com/aoughwl/aowlspt/tree/main/examples/clientprobe).
...
`hoststage` already holds [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) on most of the stand-in's compiled
