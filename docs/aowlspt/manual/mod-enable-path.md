---
title: "Getting a new mod to stay loaded"
---

# Getting a new mod to STAY loaded

::: info From the aowlspt repository — [`docs/MOD-ENABLE-PATH.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/MOD-ENABLE-PATH.md)
...
1. **The DLL is on disk**, at <code>&lt;install&gt;&#92;aowlspt&#92;mods&#92;&lt;dir&gt;&#92;&lt;lib&gt;.dll</code>.
2. **The registry names it** — an entry in [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) with a matching
   `artifact.dir` / `artifact.library`, and `"client"` in its `sides`.
3. **Something SELECTS it** — an active mod list in [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) names
...
* [`mods/manager/mgr/resolve.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/resolve.nim) walks the active lists and the user's
...
* `mods/manager/manager.nim:routeClient` serves that resolution at
  <code>GET /aowlspt/mods/client/&lt;hostver&gt;</code> as
...
<code>/aowlspt/mods/disable/&lt;id&gt;</code> switches it back off; <code>/aowlspt/mods/clear/&lt;id&gt;</code>
...
in [`mods/manager/manager.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/manager.nim). Neither in-game UI uses them:
...
[`abi/aowlspt_overlay.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_overlay.h)) and now also what the **native MODS tab's per-mod
...
list in [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) instead — that is a tracked file with a git
...
not armed. Nothing was wrong with it: it was in [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) and in no
