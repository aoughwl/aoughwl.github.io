---
title: "The reload / magazine-load path"
---

# The in-raid reload / magazine-load path — complete offline map

::: info From the aowlspt repository — [`docs/AMMO-LOADING-MAP.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/AMMO-LOADING-MAP.md)
...
abbreviated below as <code>R &lt;verb&gt;</code>. Verbs used: `find`, `typemethods`, `methods`,
...
> <code>ammoloading  ARMED but NEVER FIRED. mode=probe … LoadMagazineProcess::Start &gt; @0x768F50 has been entered ZERO times and the broad probe &gt; LoadMagazineProcess::.ctor @0x768D40 is installed, never fired</code>
...
| 6729 | <code>EFT.Player/PlayerInventoryController/&lt;LoadMagazine&gt;d&#95;&#95;51</code> |
...
| `0x7af600` | <code>ReloadCylinderMagOperation::CommitReloadWithAmmo(int, AmmoPack, Player, CylinderMagazine, Weapon, List&lt;int&gt;)</code> | 7 | UNIQUE |
...
| `0x1522090` | <code>EFT.UI.ItemUiContext::ReloadWeapon(Weapon, IEnumerable&lt;CompoundItem&gt;)</code> | 3 | UNIQUE | context-menu "reload weapon" |
...
| `0x768d40` | `…/LoadMagazineProcess::.ctor(InventoryController, Magazine, Ammo, int, bool, float)` | 7 | UNIQUE | constructed **only** from <code>&lt;LoadMagazine&gt;d&#95;&#95;51::MoveNext</code> |
...
Bound by [`mods/ammoloading/ammoloading.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/ammoloading/ammoloading.nim) as read-only PREFIX drains, all
...
* <code>input &gt; 0</code> **and** `ammo == 0` → **FAIL**. The player reloaded and no
...
* <code>input &gt; 0</code> and <code>ammo &gt; 0</code> → **PASS** for the mapping, and the census names
...
also prints, once, the receiver's <code>klass-&gt;name</code>. That read uses
`Il2CppClass.name` @`0x10`, which is **INFERRED** ([`abi/aowlspt_components.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_components.h)
