---
title: "Distribution \u2014 the final build"
---

# Distribution — the final build customers receive

::: info From the aowlspt repository — [`docs/DISTRIBUTION.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/DISTRIBUTION.md)
...
::: warning Design pass
A design for the shipped build. Nothing in it was implemented.
:::
...
[`installer/README.md`](https://github.com/aoughwl/aowlspt/blob/main/installer/README.md) and [`installer/payload/README.md`](https://github.com/aoughwl/aowlspt/blob/main/installer/payload/README.md) are the source of
...
- `aowl payload` assembles `installer/payload/aowlspt/` from what has already
  ...
  `aowlspt-host.json`, `aowlspt-backend.exe`, and <code>mods/&lt;name&gt;/</code> (each mod's
...
- `aowlspt-install.exe` (built from [`installer/src/`](https://github.com/aoughwl/aowlspt/tree/main/installer/src), pure Win32 Nimony, no
...
- The install is laid down as: mirror the vanilla client (hard-linking ~77 GB
  ...
  then write [`mods/manager/config.json`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/config.json)'s default list and `backend.json`.
  `BattlEye` and `ConsistencyInfo` are deliberately not carried across.
- **`db.json` is never shipped.** It's produced locally via
  <code>aowl importdb --from &lt;SPT install&gt; --out &lt;target&gt;&#92;aowlspt</code>, and a release
  archive ships `aowl-importdb.exe` as a standalone tool for exactly this,
  since there's no `aowl.exe` outside a dev checkout.
- [`tools/deploy.json`](https://github.com/aoughwl/aowlspt/blob/main/tools/deploy.json) verifies **7 artifacts** at deploy time by content
...
- **The live inspector is not a separate artifact today.** Its code
  ([`host/Aowlspt.Host.Il2Cpp/inspect.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Host.Il2Cpp/inspect.nim), `debugui.nim`, `invoke.nim`,
  ...
  exists standalone.
- The mod API surface ([`aowl/src/aowlspt/`](https://github.com/aoughwl/aowlspt/tree/main/aowl/src/aowlspt)) is **11 modules**:
...
directories in the existing [`installer/payload/`](https://github.com/aoughwl/aowlspt/tree/main/installer/payload) shape — no new installer
...
all 11 modules under [`aowl/src/aowlspt/`](https://github.com/aoughwl/aowlspt/tree/main/aowl/src/aowlspt) plus the root `aowlspt.nim`. This
...
1. **Nimony is not vendored anywhere in this repo or its build output.**
   [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) expects it at `%USERPROFILE%\nimony\bin\nimony.exe` and
...
2. `aowl build-mod PATH` is a thin wrapper around
   <code>nimony c --app:lib -p:&lt;path-to-api-source&gt;</code>. That invocation itself is
...
3. There's also a Windows toolchain dependency for the C backend
   (`gcc`/`lld`, referenced via `e.ucrt64` in [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim), defaulting to
...
- A **build-time** split, not just a runtime flag: a release-tier host build
  ...
  — genuinely new work, not present in [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) today.
...
`installer/`, [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim), or [`tools/deploy.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/deploy.py) checks a license key, a
...
Reuse the [`tools/deploy.json`](https://github.com/aoughwl/aowlspt/blob/main/tools/deploy.json) marker pattern exactly, extended to release
...
- A new marker list, e.g. `tools/release.json`, of the same shape as
  ...
  [`aowl/src/aowlspt/`](https://github.com/aoughwl/aowlspt/tree/main/aowl/src/aowlspt) at the tag being released — catches an API module added
...
- **Never weaken [`tools/deploy.json`](https://github.com/aoughwl/aowlspt/blob/main/tools/deploy.json) to make a release check pass** — this
