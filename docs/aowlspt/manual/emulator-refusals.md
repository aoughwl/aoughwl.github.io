---
title: "Every refusal, re-audited"
---

# Every refusal in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov), and whether its reason still holds

::: info From the aowlspt repository — [`docs/EMULATOR-REFUSALS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/EMULATOR-REFUSALS.md)
...
**Every place the emulator deliberately declines to answer, taken back to the database and re-checked to see whether the reason still holds.**

[`mods/tarkov/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/README.md) §3 lists the refusals and what each costs the player.
...
| Verdict | Rows |
|---|---:|
| **Permanent** — the data does not exist in `build/db/db.json`, in [`reference/spt-4.1-surface.json`](https://github.com/aoughwl/aowlspt/blob/main/reference/spt-4.1-surface.json), or in this process at all | 24 |
...
Six reasons are wrong. Two of them are wrong in [`mods/tarkov/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/README.md), two in
...
**Whether that reason is still true.** No. <code>bots.types.&lt;role&gt;.experience.standingForKill</code>
...
<code>bots.types.&lt;role&gt;.experience.standingForKill.&lt;difficulty&gt;</code>. The difficulty is
...
> every victim at <code>bots.types.&lt;role&gt;.experience.standingForKill.normal</code>, with
...
[`mods/tarkov/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/README.md) does not mention it.
...
(<code>Dictionary&lt;MongoId, RewardDetails&gt;</code> in the dump), i.e. SPT's
...
exact failure mode [`mods/tarkov/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/README.md) §6.3 names as the one the server
...
* `RaidSettings.TimeAndWeatherSettings` carries <code>Nullable&lt;Int32&gt; HourOfDay</code> and
...
<code>locations.&lt;map&gt;.base.EscapeTimeLimit</code> — present on all 19 maps; Customs 40,
...
>   inside <code>22 -&gt; 07</code>. A request carrying only those is refused rather than
...
<code>templates.items.&lt;id&gt;.&#95;props.ammoCaliber</code> is present on every weapon, and all six
...
<code>traders.&lt;id&gt;.base.repair.currency&#95;coefficient</code> **is** the rate, and it sits next
...
|---|---|---|---|
| **Clothing unlocks unenforced, and Ragman's clothing shop unserved** (`GetTraderSuits`, `CustomizationBuy`) | the whole clothing shop is a screen that cannot be opened; every wardrobe entry is free | <code>traders/&lt;id&gt;/&#123;suits,bearsuits,usecsuits&#125;.json</code> — **3 files, 0.16 MB** — which `aowl importdb` deliberately skips (`docs/IMPORTDB.md:117`). Verified absent: no trader in the database has a `suits` key | one importer row, one route, one item-event arm. The **largest player-visible screen** on this whole list |
...
3. **`equipmentInclusive` / `equipmentExclusive` is dead code**
   ...
   [`mods/tarkov/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/README.md) §3 among the qualifiers that cost the player
...
* **The gym's stated unknown is smaller than the README says.**
  [`mods/tarkov/README.md`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/README.md) §3 says of `GymEffectivity` that "nothing says whether
...
* `build/db/db.json`, 39.40 MiB — the tables `templates.{items, handbook,
  ...
  `bots`.
* [`reference/spt-4.1-surface.json`](https://github.com/aoughwl/aowlspt/blob/main/reference/spt-4.1-surface.json), 1,704 type entries — members only, no method
  bodies and no route strings, which is why three verdicts above end at "one
  raid" or "one open-source file".
* `mods/tarkov/emu/*.nim` and [`mods/tarkov/tarkov.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/tarkov.nim) — 193 `problems.add`
