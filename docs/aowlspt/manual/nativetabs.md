---
title: "nativetabs \u2014 one declarative call"
---

# nativetabs — one declarative call adds a settings tab

::: info From the aowlspt repository — [`docs/NATIVETABS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/NATIVETABS.md)
...
**Status of every number here:** measured. RVAs from [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py)
against `D:\Aowlspt\GameAssembly.dll` + `.cache/global-metadata.dec.dat`;
offsets from `Il2CppMetadataRegistration.fieldOffsets` ([`tools/fldoff.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/fldoff.py),
...
| <code>SettingsTab.&#95;createdControls : List&lt;SettingControl&gt;</code> | `@0x88` | — |
...
<code>SettingsTab.CreateControl&lt;T&gt;(T prefab, Transform parent)</code> has exactly **one**
instantiation:
...
<code>method&lt;object&gt;</code> is the shared-reference instantiation, so it serves every
...
<code>SettingDropDown::BindTo&lt;T&gt;</code> has 7 instantiations;
<code>method&lt;int&gt;</code> is `0x2B81640`, `body-unique-in-genericMethodPointers`. Also
...
<code>shared &lt;RVA&gt;</code> reports **UNKNOWN** for all generic bodies. That is a refusal,
...
<code>GameSetting&lt;T&gt;</code>. Constructing either needs type injection or an instantiated
generic layout, and **<code>GameSetting&lt;T&gt;</code> has no reachable layout offline** — all
...
2. **Read the `MethodInfo*` for `CreateControl`/`BindTo` out of a caller's
   ...
   <code>AddComponent&lt;T&gt;</code>), which unlocks §2.4 and §2.5 and lets the game do its own
...
## 6. Verification — [`tools/nativetabs_check.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/nativetabs_check.py)
