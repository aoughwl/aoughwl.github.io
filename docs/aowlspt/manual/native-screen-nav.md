---
title: "Native menu-screen navigation"
---

# Native menu-screen navigation (offline metadata study)

::: info From the aowlspt repository — [`docs/NATIVE_SCREEN_NAV.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/NATIVE_SCREEN_NAV.md)
...
All RVAs below produced by <code>tools/il2cpp&#95;resolve.py D:/Games/Tarkov/GameAssembly.dll .cache/global-metadata.dec.dat &lt;verb&gt;</code>; every one byte-verified with `bytes` and
classified with `shared`. [`tools/fldoff.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/fldoff.py) self-check
...
<code>MainMenuShowOperation.&lt;Matchmaker&gt;k&#95;&#95;BackingField @ 0x118</code> (measured, fldoff).
...
<code>&lt;ShowMatchmakerSideSelectionInternal&gt;d&#95;&#95;34::MoveNext</code> @ `0xA345C0`:
...
real body. Reached by tail-jump from <code>&lt;Awake&gt;b&#95;&#95;41&#95;3</code> (`0x1792490`) and
<code>&lt;Awake&gt;b&#95;&#95;41&#95;4</code> (`0x1792530`), which are the toggle handlers. (measured)
...
<code>RaidSettings = &#91;this+0x70&#93;; if (RaidSettings) RaidSettings-&gt;&#91;0x20&#93; = side;</code> (measured)

Cross-checked with fldoff:
* `RaidSideSelectionScreenController.RaidSettings @ 0x70` (measured)
* <code>EFT.RaidSettings.&lt;Side&gt;k&#95;&#95;BackingField @ 0x20</code>, type `ESideType` (measured)
...
`ESideType`; `RaidSettings.SelectedGameModes@0x10` (<code>Dictionary&lt;string,bool&gt;</code>) and
...
<code>&lt;Ready&gt;d&#95;&#95;40::MoveNext</code> @ `0xA32E30` (the body of `MatchmakerOperation::Ready`) calls,
in order (measured):
...
|---|---|---|---|
| 1 | Does `OnReadyPressed` work when the location screen was never shown (i.e. is `LocationId` alone enough for `ResolveByPlayerLevel`)? | **INCONCLUSIVE** | Live inspector: `read $mmop+0x40 ptr`, then <code>read &lt;rs&gt;+0x30 str</code> after the flow reaches side-select. If `LocationId` is already non-null at side-select, step 4 is unnecessary; if null, write it and `call` `0xA312E0`, then read <code>&lt;rs&gt;+0xa8</code> (`_selectedLocation`) back — non-null = the state was really committed. |
...
| 4 | Does `Ready()` need `MatchmakerPlayersController` to be initialised (it calls `SetProcessingReadyButtonPress` first)? | **INCONCLUSIVE** | Read `mmOp+0xa0` (<code>&lt;MatchmakerPlayersController&gt;k&#95;&#95;BackingField</code>) before calling; refuse if null. |
