---
title: "The live inspector \u2014 verb reference"
---

# The aowlspt live inspector — verb reference

::: info From the aowlspt repository — [`docs/INSPECTOR-VERBS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/INSPECTOR-VERBS.md)
...
`ATOM` is <code>0x&lt;hex&gt;</code>, a decimal, or `$name`. `+` **moves** the address; `@`
...
`OP` is `==` `!=` <code>&lt;</code> <code>&lt;=</code> <code>&gt;</code> <code>&gt;=</code>. For `TYPE str` it is `==` `!=` or
...
`$s0..$sN`, with **this** node marked <code>&lt;&lt;&lt; THIS</code> and its sibling index stated.
...
It is not a new transport. It imports [`tools/ichannel.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/ichannel.py), which is now a
re-export of [`plugins/aowlsptcode/mcp/channel.py`](https://github.com/aoughwl/aowlspt/blob/main/plugins/aowlsptcode/mcp/channel.py) — the sentinel-guarded
transport and prose parsers the `aowlinspect` MCP server already uses.
[`tools/inspector.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/inspector.py) (under `ui.py` / `enterraid.py`) delegates to the same
module now, so the python side is **one** implementation where it was three.
The Nim side ([`tools/aowlui.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowlui.nim)'s `Ui`) was already shared by `aowl ui`,
...
that is where mod state lives (`docs/MOD-ENABLE-PATH.md`, [`mods/manager`](https://github.com/aoughwl/aowlspt/tree/main/mods/manager)).
Nothing here touches the Unity main thread. Implementation: [`tools/modctl.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/modctl.py).

| verb | what it does |
|---|---|
| `mods [FILTER]` | every mod, with **which of the four gates is blocking it** |
| <code>modinfo &lt;id&gt;</code> | one mod in full, with the manager's own reason |
| <code>modenable &lt;id&gt;</code> | persistent user override ON — needs `.write on` |
| <code>moddisable &lt;id&gt;</code> | persistent user override OFF — needs `.write on` |
| <code>modreload &lt;id&gt;</code> | live restart — **refuses by name** (see below) |
...
|---|---|---|
| FAIL | `DLL_MISSING` | gate 1 — no <code>mods&#92;&lt;dir&gt;&#92;&lt;lib&gt;</code> in the install |
...
not this: it re-reads [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json) from disk. It does not quiesce the
