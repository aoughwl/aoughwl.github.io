---
title: "The pre-1.0 database against post-1.0"
---

# The pre-1.0 database against post-1.0's own tables

::: info From the aowlspt repository — [`docs/POST1-DATA-DELTA.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/POST1-DATA-DELTA.md)
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) serves a database imported from a **pre-1.0 SPT install**
([IMPORTDB.md](./importdb)) to a **post-1.0 client** (build 1.1.0.1.46777).
...
| locations | `/client/locations` | `large/134.json` | 1.7 MB | <code>locations.&lt;name&gt;.base</code> |
...
| traders | `/client/trading/api/traderSettings` | `responses/071.json` | 39.7 KB | <code>traders.&lt;id&gt;.base</code> |
...
`D:\SPT` on 2026-08-19 — the same file [IMPORTDB.md](./importdb) describes.
...
one member. Nothing in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) reads it (§6).
...
an <code>&lt;id&gt; Name</code> key in `locales.global.en`. All but 6 have one in BSG's locale.
That is the mechanism behind the "31 templates render as their id" row in
[BACKLOG.md](./backlog), scaled up: importing the new items without the new
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) reads it. The other 63 are additions — the new tear-gas health
...
the key up in <code>/client/locale/&lt;lang&gt;</code>. The keys are the same strings, so an
...
is priced off the handbook by deliberate choice ([IMPORTDB.md](./importdb)),
...
dialogue. SPT has no counterpart: its <code>traders/&lt;id&gt;/dialogue.json</code> (4 traders)
...
[IMPORTDB.md](./importdb) lists as not imported. So the emulator ships an empty
...
`mods/tarkov/emu/*.nim` and [`mods/tarkov/tarkov.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/tarkov.nim) were extracted and
...
| removed | table | read by [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)? |
|---|---|---|
...
- **`emu/health.nim:241` — <code>effects&#95;health.&lt;factor&gt;.value</code>**, documented at
...
| <code>traders.&lt;id&gt;.base</code> | +6 traders | **base only.** The capture has no assort for the six, so they would be traders with nothing to sell. Better as a known gap than as six empty shops. |
| <code>locations.&lt;map&gt;.base</code> | +5 maps, +10 fields | `Sandbox_start` is the only one whose full base was captured. `Icebreaker`, `Terminal_ui`, `laboratory_dark`, `Lighthouse2` appear only in the trimmed list — no `SpawnPointParams`, no `exits`, no `waves`. A map with no spawn points is not a map. **Do not import these**, and fix `terminal`'s `_Id` from `5704e5a4d2720bac5b8b4567` to `65cc8f81a9aac3e77d0cfd3e` instead, which is one value. |

**Must stay SPT-derived, permanently:**

`bots.*` · <code>locations.&lt;map&gt;.looseLoot</code> / `staticLoot` / `staticContainers` /
`staticAmmo` · <code>traders.&lt;id&gt;.assort</code> / `questassort` · `hideout.*` ·
...
[`tools/importdb.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/importdb.nim) reads a directory of SPT JSON and writes `db.json`. The
...
(`tools/importdb.nim:669`) already asserts the 29 paths [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) reads
...
gap [BACKLOG.md](./backlog) already names, it needs no importer change at all,
