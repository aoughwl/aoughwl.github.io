---
title: "Importing a real database"
---

# Importing a real database

::: info From the aowlspt repository — [`docs/IMPORTDB.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/IMPORTDB.md)
...
The emulator answers the client out of <code>&lt;root&gt;&#92;db.json</code>. Every database it had
...
| `traders/*/` | 39 of 44 | 2.11 MB | 1.4 MB | <code>traders.&lt;id&gt;.base</code> / `.assort` / `.questassort` / `.dialogue` — 12 traders, 5,790 assort items. Only 4 traders ship a `dialogue.json`; it is the insurance-message locale ids `emu/dialogue` reads, not chat lines |
| `bots/` | 59 | 8.5 MB | 5.9 MB | `bots.core`, `bots.base`, <code>bots.types.&lt;role&gt;</code> — 57 roles |
| `locations/*/base.json` | 19 | 6.7 MB | 1.4 MB | <code>locations.&lt;name&gt;.base</code> |
| `locations/*/staticLoot.json` | 13 | 12.1 MB | 7.7 MB | <code>locations.&lt;name&gt;.staticLoot</code> |
| `locations/*/staticContainers.json` | 13 | 5.9 MB | 3.3 MB | <code>locations.&lt;name&gt;.staticContainers</code> |
| `locations/*/staticAmmo.json` | 13 | 0.2 MB | 0.2 MB | <code>locations.&lt;name&gt;.staticAmmo</code> |
| **`locations/*/looseLoot.json`** | 13 | **575 MB** | **575 MB** | <code>locations.&lt;name&gt;.looseLoot</code> — **opt-in**, see below |
...
|---|---:|---:|---|
| `locations/*/looseLoot.json` | 13 | 574.72 MB | **yes** — `emu/loot` reads <code>locations.&lt;map&gt;.looseLoot</code>. Opt-in per map, `--loose`; see *Size, measured* |
| `locales/global/*` other than `en` | 16 | 50.39 MB | **yes** — `emu/templates` reads <code>locales.global.&lt;lang&gt;</code>. Opt-in, `--locales` |
| `templates/dialogue.json` | 1 | 6.26 MB | no. SPT's chat-command templates, `{elements:[...]}`. The trader lines the emulator *does* use are <code>traders/&lt;id&gt;/dialogue.json</code>, which is imported |
...
the paths [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) reads with `dbRead` line up nearly key for key. Four
...
2. **`locales/menu/en.json` is `{menu:{...}}`.** <code>/client/menu/locale/&lt;lang&gt;</code>
   returns <code>locales.menu.&lt;lang&gt;</code> verbatim, so the wrapper comes off here or
...
from <code>locations.&lt;map&gt;.staticAmmo</code>, weighted by the map's own
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) inside one day — `hideout.customisation`, `hideout.qte`,
...
answers "not there", every reader in [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) treats that as "this
...
So the self-check now asserts it. `checkReadPaths` in [`tools/importdb.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/importdb.nim)
carries the inventory of the 29 database paths [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) reads with a
...
`grep` line in the comment above the table when [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) grows a route.
...
have no <code>&lt;id&gt; Name</code> in the English locale and render as their id, which is ugly
and not broken.
...
2. **If loose loot ever wants to be the default, the fix is in the emulator,
   ...
   [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)'s call to make, and until it is made this flag is not the
...
**written against [`tests/fixtures/emu-full.json`](https://github.com/aoughwl/aowlspt/blob/main/tests/fixtures/emu-full.json), and it shows** — which is a
...
- **`emutest`** completes, and a third of its checks fail against real data.
  ...
  The right tool against real data is [`tools/realtest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/realtest.nim), which discovers
...
Each row is one run of <code>aowl-importdb --from D:&#92;SPT &lt;flags&gt; --out &lt;scratch&gt;</code>
...
puts everything under <code>&lt;target&gt;&#92;aowlspt</code>, and that is what the backend is given
