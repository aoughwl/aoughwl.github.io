...
::: info From the aowlspt repository — [`docs/WIRE.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/WIRE.md)
...
applied ascending for `i` in <code>1 ..&lt; len</code> to unshuffle, and descending to
...
the allocation, and [`tests/bsgwiretest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tests/bsgwiretest.nim) asserts the two agree.

This is the same permutation the repository already used for `/client/metadata`
response bodies ([`tools/metablob.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/metablob.py)). The discovery was not the permutation —
...
[`tools/bsgwire.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/bsgwire.py) decodes anything from a capture:
...
[`tools/bsgaes.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/bsgaes.py) is the AES, standard library only (this machine has no pip),
checked against the FIPS-197 known-answer vectors for all three key sizes.

## Reading a body, in the server

[`backend/wire.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/wire.nim) has `bsgUnshuffle`, `bsgShuffle` and `bsgUnwrap`, and
...
sessions as a `.saz`, and decode with [`tools/bsgwire.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/bsgwire.py). If the key has moved,
it is recovered by disassembling `HTTPTransportManager::.cctor` — see
[`tools/METABLOB.md`](https://github.com/aoughwl/aowlspt/blob/main/tools/METABLOB.md) for how the metadata is decrypted first, since that is what
