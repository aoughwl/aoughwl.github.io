---
title: "The settings screen map"
---

# The post-1.0 Tarkov settings screen — complete offline map

::: info From the aowlspt repository — [`docs/SETTINGS-UI-MAP.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/SETTINGS-UI-MAP.md)
...
abbreviated below as <code>R &lt;verb&gt;</code>. Verbs used: `find`, `fields`, `typemethods`,
...
| **GENERIC** | <code>&lt;Destroyed&gt;k&#95;&#95;BackingField</code> | bool | ``BaseScreen`3`` |
...
| `0x0` (**static**) | `_tabs` | <code>Dictionary&lt;ESettingsGroup,SettingsGroupObjects&gt;</code> | SettingsScreen |
...
`ScreenController@0x90`, <code>&lt;Destroyed&gt;k&#95;&#95;BackingField@0x98</code> — derived from use.
...
| <code>CreateControl&lt;T&gt;(T prefab, Transform parent)</code> | **generic body `0x2B86E20`** | §5 T5 |
...
| `0xe8`..`0x120` | eight <code>BlockBySettingController&lt;T&gt;</code> | vsync / framerate / dlss-on / dlss-off / fsr2 / fsr3 / reflex / sampling |
...
LIVE confirms the shape: <code>Control Settings &gt; Toggles &gt; &#123;ControlToggle, GesturesToggle&#125;</code> beside <code>Control Settings &gt; Content &gt; &#123;ControlsPart, GesturesPanel&#125;</code>.
...
MEASURED <code>List&lt;T&gt;</code> runtime layout, read out of `NotifyToggleOn`'s own code
...
|---|---|---|
| `SettingToggle` (15677) | `Toggle : UpdatableToggle` | <code>BindTo(GameSetting&lt;bool&gt;)</code> `0x16FD0D0` UNIQUE |
| `SettingFloatSlider` (15662) | `Slider : NumberSlider` | <code>BindTo(GameSetting&lt;int&gt;,int,int)</code> `0x16FB4C0`; <code>BindTo(GameSetting&lt;float&gt;,float,float,string)</code> `0x16FB810` — both UNIQUE |
...
<code>&lt;KeyGroup&gt;k&#95;&#95;BackingField@0x108</code>, `OnKeyPressedAction@0x110`,
`OnKeyChosenAction@0x118`, `_interactable@0x120`, `_isEmpty@0x121`,
`_pressTypes@0x128`.
<code>Show(KeyGroup, Dictionary&lt;string,EPressType&gt;, bool)</code> `0x16F9340`;
...
<code>&lt;AxisGroup&gt;@0x80</code>, <code>&lt;Positive&gt;@0x88</code>, `OnKeyPressedAction@0x90`,
...
`_localeUnsubscriber@0x98`, <code>&lt;FormattedText&gt;k&#95;&#95;BackingField@0xa0</code>.
...
enters `ShowScreen`, not `OpenGroup`. <code>&lt;Awake&gt;b&#95;&#95;2()</code> @`0x17240D0` is the second
closure (reads `this.value@0x18`); <code>&lt;Awake&gt;b&#95;&#95;26&#95;0()</code> @`0x1721900` is a separate
non-capturing lambda on the screen itself.

<code>&lt;&gt;c&#95;&#95;DisplayClass26&#95;0</code> is type index **15731**, and the name is AMBIGUOUS —
...
2. <code>for (i = 0; i &lt; m&#95;Toggles.&#95;size@0x18; i++)</code>:
...
<code>&lt;DisplayRevertMessage&gt;b&#95;&#95;41&#95;0</code> `0x1721950`.
...
in this exact order, each followed by <code>tab.&lt;virtual klass+0x288&gt;()</code> = `Close()`:
...
<code>&lt;IsInitialized&gt;@0x90 = false</code> → `CompositeDisposable::Dispose` `0x1D252C0`.
...
exactly **one** instantiation, <code>class&lt;&gt; method&lt;object&gt;</code>, body RVA **`0x2B86E20`**
...
`SettingControl` API: <code>CreateControl&lt;T&gt;(template, parent)</code> →
...
So the subtab <code>onValueChanged -&gt; HandleGesturesToggle</code> edge is **NOT
...
SHARED generic <code>UnityEvent&#96;1&lt;bool&gt;::AddListener</code>, which a rel32 callgraph
...
<code>&lt;isPointerInside&gt;@0xf0</code>, <code>&lt;isPointerDown&gt;@0xf1</code>, <code>&lt;hasSelection&gt;@0xf2</code>;
...
<code>&lt;&gt;c&#95;&#95;DisplayClass26&#95;0::.ctor</code>, and `TMP_Text::ForceMeshUpdate`. A resolve that
...
**T5 — <code>SettingsTab::CreateControl&lt;T&gt;</code> is a SHARED GENERIC.** MEASURED
`R genericmethods`: one body, `0x2B86E20`, <code>method&lt;object&gt;</code>. MEASURED
...
**T11 — <code>&lt;&gt;c&#95;&#95;DisplayClass26&#95;0</code> is an ambiguous type name.** MEASURED: 23 types
...
body), and it is **mandatory and non-NULL** only for <code>CreateControl&lt;T&gt;</code>.
...
2. `ShowScreen(screen, group, NULL)` — `rcx = SettingsScreen*`,
   ...
   `r8 = NULL`. Source: <code>&lt;Awake&gt;b&#95;&#95;1</code> passes exactly this
...
9. **`nativeui.nim`'s target table** (67 entries) carries `Toggle::Set`,
   ...
   API. ([`abi/aowlspt_bridge.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_bridge.h) and [`abi/aowlspt_navui.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_navui.h) do carry the first two
...
* **Whether the subtab `onValueChanged` edge really is prefab-serialized.**
  ...
  read: <code>component &lt;GesturesToggle GO&gt; AnimatedToggle</code>, then
...
* **`SettingsTab::OnTabSelected` `0x171BDA0` appears to have no caller.**
  ...
  `R callers 0x171bda0`.
* **`ScreenController@0x90` / <code>&lt;Destroyed&gt;@0x98</code>.** Derived from use; the
  offline field table reports both as `GENERIC` and cannot confirm the split.
  A live <code>read &lt;SettingsScreen&gt;+0x90 ptr</code> followed by `parent`/`fields` on the
...
* **The exact `Show*()` → <code>CreateControl&lt;T&gt;</code> call shape.** §2.9 gives the
