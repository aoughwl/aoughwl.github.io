...
::: info From the aowlspt repository — [`docs/EMULATOR.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/EMULATOR.md)
...
[`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) is a Tarkov server — profiles, the static tables, traders and
...
| **Insurance** | premiums off the handbook, buying the cover, and returns that survive a restart — with the covering letter **in the trader's own words**, resolved out of <code>traders.&lt;id&gt;.dialogue</code> through the locale the client last asked for |
...
invariant [`tools/soak.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/soak.nim) holds it to is that the money is in the fewest
...
- **A hideout upgrade** resolves the stage's `requirements` — the items, the
  ...
  answered is [`tools/realtest.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/realtest.nim), which now buys what a trader or the flea
...
- The mailbox is **bounded**. An emptied message is kept — `rewardCollected`,
  ...
  removed one at all until it was measured: <code>mail.&lt;id&gt;</code> grew 267 bytes per
...
- The scav's stash is **not stored**. "Shared stash" implemented by copying the
  ...
  <code>scav.&lt;pmc id&gt;</code> is only what is the scav's; the PMC's stash is spliced in at
...
Where the live dump comes from is [IMPORTDB.md](./importdb):
...
(`tools/importdb.nim:679`), which walks the 29 database paths [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov)
...
The database is [`tests/fixtures/emu-full.json`](https://github.com/aoughwl/aowlspt/blob/main/tests/fixtures/emu-full.json) — a trader with an assort, a
...
[EMULATOR-COVERAGE.md](./emulator-coverage) is the gap list: every operation the
...
served. Those three numbers are derived, not stored — [`tools/coverage.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/coverage.nim) reads
the routes [`mods/tarkov/tarkov.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/tarkov.nim) registers and puts a row in the *empty*
...
friends, the SPT launcher's own routes, trader clothing (<code>trader/&lt;id&gt;/ suits.json</code> is not imported), cultist recipes (`hideout.production.cultistRecipes`
...
and its buffer. <code>/client/notifier/getwebsocket/&lt;session&gt;</code> is upgraded and held,
...
- **Reading a hook's arguments used to be refused** on the client side. It is
  ...
  in [MODDING.md](./modding) and [IL2CPP.md](./il2cpp).
...
`distance` and `daytime` keys with neutral values (<code>&gt;= 0</code>, `0..0`) that restrict
