...
::: info From the aowlspt repository — [`docs/RELEASE-1.0-READINESS.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/RELEASE-1.0-READINESS.md)
...
**A read-only audit of what was and was not ready at 1.0. Nothing was built or run in this pass; that is the point of it.**
...
those commands do are from reading [`tools/aowl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowl.nim) directly, not from executing
...
[`tools/aowlsession.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowlsession.nim) documents `POST /aowlspt/tarkov/launcher/profile/create`,
which is what `--new` calls. Verified in [`tools/aowllaunch.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowllaunch.nim): when a fresh
...
**game's own in-client character-creation flow** ([`tools/aowllaunch.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowllaunch.nim) ~1888).
...
* **#1, #2, #3 — FIXED in `958e7a2`.** All five unregistered mods
  ...
  [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json). `settingshub` and `uihub` went into
  ...
  **Verified**: <code>installeruildowl-regcheck.exe --repo &lt;repo&gt;</code> — 200
...
* **Profile creation — VERIFIED LIVE, no longer read-only.** The audit could
  ...
  profile was then deleted (<code>store/aowl.tarkov/profile.&lt;id&gt;</code> plus its `.hist`
...
2. Run <code>aowl-importdb.exe --from &lt;SPT install&gt; --out &lt;target&gt;&#92;aowlspt</code> to
...
- Whether `aowlspt-backend.exe` degrades gracefully or hard-fails with no
  `db.json` present.
- Whether the installer's default [`mods/manager/config.json`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/config.json) seed actually
