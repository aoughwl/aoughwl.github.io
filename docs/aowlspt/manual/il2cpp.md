---
title: "The post-1.0 client host"
---

# The post-1.0 client host

::: info From the aowlspt repository — [`docs/IL2CPP.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/IL2CPP.md)
...
::: info Predates the token-gate discovery
The reflection surface this page describes was later found to be **token-gated**: calling those exports with the stock signature returns a plausible random number rather than failing. Read [Traps and hard-won facts](/docs/aowlspt/pitfalls) and [the interaction layer](./interaction-layer-map) before acting on the `resolve` / `call` rows here.
:::
...
server mod beside it. [`host/Aowlspt.Host.Il2Cpp`](https://github.com/aoughwl/aowlspt/tree/main/host/Aowlspt.Host.Il2Cpp) is the post-1.0 answer.
...
[`aowl/src/aowlspt/il2cpp.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/il2cpp.nim) is the binding. [`abi/aowlspt_shim.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_shim.h) holds the
...
impersonating a Windows component. [`abi/aowlspt_inject.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_inject.h) has the mechanism.
...
[`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) pins both directions against
...
value type goes through it. [`mods/sain/client/live.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/sain/client/live.nim) does the same for the
same reason.
...
[`tests/detour_test.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_test.c) exercises the path directly: that the postfix dispatcher
...
- **Nothing is allocated, on either side.** No payload, no GC handle for a
  ...
  measured: [`examples/highlevel`](https://github.com/aoughwl/aowlspt/tree/main/examples/highlevel) fires twenty thousand typed firings and checks
...
[`tests/detour_test.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_test.c) pins the frame's byte offsets to the thunk's own assembly
...
fired for was on the boxed path at about a microsecond a call. [`mods/fov`](https://github.com/aoughwl/aowlspt/tree/main/mods/fov)
...
right now. Measured against [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp): **8 ns**, against **1112 ns**
...
handler ran with this method's arguments. [`tests/detour_race.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_race.c) saw exactly
...
[`examples/clientprobe`](https://github.com/aoughwl/aowlspt/tree/main/examples/clientprobe) is a worked version. Note what it does with timing: it
...
[`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp) — a stand-in that exports the same C API as
`GameAssembly.dll`.
...
[MODDING.md](./modding). This used to point at the one verdict that came out
...
The host starts [`host/Aowlspt.Overlay`](https://github.com/aoughwl/aowlspt/tree/main/host/Aowlspt.Overlay) on its boot thread, after the mods are
...
[PERF.md](./perf). They are against the stand-in runtime, not BSG's.
