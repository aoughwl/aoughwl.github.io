---
title: "DLSS-NR on this install"
---

# DLSS-NR ("DLSS 5", `nvngx_dlssnr.dll`) on this install -- the Turing / D3D11 route

::: info From the aowlspt repository — [`docs/DLSSNR.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/DLSSNR.md)
...
source), and what is **inferred**. The tool is [`tools/dlssnr.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/dlssnr.py); its self-test
is registered in [`tools/selftests.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/selftests.py). `docs/DLSS.md` section 5 is the earlier
...
<code>driver 581.42 -&gt; TOO OLD</code>; the gate fired as designed.)
...
cases. Kernel names come from the <code>.text.&lt;name&gt;</code> sections and were
...
per-kernel <code>.nv.info.&lt;k&gt;</code> sections were not decoded.
...
* **`.text`, 29 bytes in two places, decoded by hand:**
  1. **RVA `0x17ecc`-`0x17ee7`, the GPU architecture gate.** The function
     at `0x17e20` (it logs <code>DLSSNR: Created feature %u (... preset=%d -&gt; %s)</code>) reads the NvAPI architecture id, does `lea ecx,[eax-0x140]; cmp
     ecx,0x70; ja default; jmp [table+rcx]`. In NVIDIA's build the cases
     `0x140`, `0x160` (Turing), `0x170` (Ampere), `0x180`, `0x190` (Ada) and
     `0x1A0` each do <code>mov esi,&lt;id&gt;; jmp 0x17efc</code>, and `0x17efc` is <code>mov &#91;rsp+0x28&#93;,0x1B0; mov &#91;rsp+0x20&#93;,esi; lea r9/r8/rcx,&lt;strings&gt;</code> = the
