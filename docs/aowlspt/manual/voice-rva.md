---
title: "Voice \u2014 offline IL2CPP archaeology"
---

# VOICE_RVA — offline IL2CPP archaeology for two voice features

::: info From the aowlspt repository — [`docs/VOICE_RVA.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/VOICE_RVA.md)
...
Tool: [`tools/il2cpp_resolve.py`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cpp_resolve.py) (verbs `type`/`methods`/`shared`/`bytes`) and
`tools/fldoff.py fields`. Every command is <code>il2cpp&#95;resolve.py GAMEASM METADEC &lt;verb&gt; …</code>.
...
|---|---|---|---|
| <code>BotTalk::Say(EPhraseTrigger type, bool sayImmediately, Nullable&lt;ETagStatus&gt; additionalMask, ETagFilter tagFilter)</code> | `0x1a892f0` | **UNIQUE** (owners=1) | real prologue `48 89 5C 24 20 55 56 57 48 81 EC 90 00 00 00 …` |
...
| <code>BotTalk::TrySay(EPhraseTrigger, Nullable&lt;ETagStatus&gt;, bool)</code> | `0x1a898a0` | UNIQUE | |
...
- R9 = <code>Nullable&lt;ETagStatus&gt;</code> — a nullable value type; passed by pointer/expanded per ABI (verify frame live before *calling* it; for a **prefix suppress detour you only inspect/early-return**, so the exact R9 packing does not matter to gate it)
