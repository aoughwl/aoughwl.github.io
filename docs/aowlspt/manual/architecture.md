---
title: "The shape of aowlspt"
---

# The shape of aowlspt

::: info From the aowlspt repository — [`docs/ARCHITECTURE.md`](https://github.com/aoughwl/aowlspt/blob/main/docs/ARCHITECTURE.md)
...
> client host has been exercised against [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp), a stand-in that
...
[`host/Aowlspt.Host.Il2Cpp`](https://github.com/aoughwl/aowlspt/tree/main/host/Aowlspt.Host.Il2Cpp), a nimony DLL injected into the running game
process. Both implement the same C ABI, [`abi/aowlspt_abi.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_abi.h), and refuse the
capabilities they cannot honestly provide rather than faking them. **The Tarkov
server is itself a mod** — [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov), on the same ABI as everyone else's —
...
[`abi/aowlspt_abi.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_abi.h) states it and [ABI.md](./abi) explains the decisions. Five
...
`AOWLSPT_ABI_REVISION` in [`abi/aowlspt_abi.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_abi.h)). The version is the breaking
...
[`tests/abi_layout.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/abi_layout.c) pins every one of them under the real compiler and
[`tests/abi_layout.nim`](https://github.com/aoughwl/aowlspt/blob/main/tests/abi_layout.nim) asserts nimony agrees; both are the first two gates of
`aowl test`.
...
[ABI.md](./abi) has the argument and the alternative that was not taken.
...
`exportMod` in [`aowl/src/aowlspt.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt.nim) writes all three. A mod's `AowlModInfo`
...
[`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) is the loader, and both nimony hosts share it:
...
would be far worse than one that loaded too much. [MODMANAGER.md](./modmanager)
...
`aowl_hostapi_new` (in [`abi/aowlspt_shim.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_shim.h)) builds the block for every nimony
...
- the IL2CPP client host calls `aowl_hostapi_arm_live` and then
  `aowl_hostapi_arm_typed` ([`abi/aowlspt_live.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_live.h), which only it includes) to
  publish `handle_pointer`, `handle_pin` and `patch_typed` for real, ending at
  216;
- the backend calls `aowl_hostapi_arm_notify` ([`abi/aowlspt_notify.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_notify.h), which
...
- `aowlspt-sim` does the same trick for the same reason with
  `aowl_hostapi_arm_sim` ([`host/Aowlspt.Sim/aowlsim.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Sim/aowlsim.nim)): three refusing
...
minutes. It was C# until it was rewritten as [`host/Aowlspt.Sim/aowlsim.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/Aowlspt.Sim/aowlsim.nim),
and the rewrite is why it has a store: it loads mods through the same
[`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim) the other two use and answers `store_*` out of the
same [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim) the backend writes profiles with, so a mod that
persists anything — [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) above all — can be simulated at all. What
...
- The mod entry points, by the `exportMod` macro in [`aowl/src/aowlspt.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt.nim).
- [`reference/spt-4.1-surface.json`](https://github.com/aoughwl/aowlspt/blob/main/reference/spt-4.1-surface.json) and `.txt`, by [`tools/SptReflect`](https://github.com/aoughwl/aowlspt/tree/main/tools/SptReflect) — SPT's
...
- `build/db/db.json`, by `aowl importdb --from D:\SPT` — the real game's tables,
  ...
  release. See [IMPORTDB.md](./importdb).
...
- The generated regions of `docs/EMULATOR-COVERAGE.md`, by
  [`tools/coverage.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/coverage.nim) (`aowl-coverage`). It computes the four sweeps that
  document's "How it was derived" section states — in nimony, by reading
  `reference/spt-4.1-surface.txt` and [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov) — and it derives the
  served / empty / absent split from `docs/coverage-rows.json`, one row per
  callback method, joined against the routes [`mods/tarkov/tarkov.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/tarkov/tarkov.nim) actually
...
workers (`AOWL_NET_WORKERS`, [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h)). The poller owns the listening
...
(`aowl_session_lock` / `aowl_session_unlock`, [`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h)), across the
...
cannot tell a working route from a broken one and why [`tools/aowlprobe.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowlprobe.nim)
...
[BACKEND.md](./backend) is the writer's guide and [PERF-SERVER.md](./perf-server)
...
[`tools/il2cppprobe.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/il2cppprobe.nim) was run against, 242 functions. `il2cpp_class_from_name`,
...
as the mods it hosts. [`aowl/src/aowlspt/il2cpp.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/il2cpp.nim) is the binding.
...
[`tools/aowllaunch.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/aowllaunch.nim) starts the game suspended, `LoadLibrary`s the host into
...
in [`abi/aowlspt_lock.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_lock.h) — coarse on purpose, for the reason that header gives.
...
[PERF.md](./perf), "What the host costs per frame"; this page deliberately does
...
[IL2CPP.md](./il2cpp) is the whole of it.
...
**The fast path** is [`aowl/src/aowlspt/fast.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/fast.nim). A mod *binds* a method or a
...
Measured by [`tools/perfbench.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/perfbench.nim) against [`tests/mockil2cpp`](https://github.com/aoughwl/aowlspt/tree/main/tests/mockil2cpp): a boxed call is
...
[`abi/aowlspt_detour.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_detour.h): it decodes enough of the prologue to relocate it into a
...
[PERF.md](./perf) has the full tables, the provenance, and what they do not
prove.
...
here — [`mods/tarkov`](https://github.com/aoughwl/aowlspt/tree/main/mods/tarkov), guid `aowl.tarkov` — and it is an ordinary mod. It imports
...
[`backend/aowlbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/aowlbackend.nim) therefore has no game routes at all. Its route table
...
The same argument produces [`mods/manager`](https://github.com/aoughwl/aowlspt/tree/main/mods/manager) (`aowl.manager`), which is also just a
mod: it reads [`registry/mods.json`](https://github.com/aoughwl/aowlspt/blob/main/registry/mods.json), resolves it against your selection, and
serves the answer under `/aowlspt/mods/...`.

[EMULATOR.md](./emulator) is what the emulator implements and refuses;
[EMULATOR-COVERAGE.md](./emulator-coverage) is the gap list, derived from
`reference/` rather than from memory.

## The store: atomic, write-through, and what that costs

<code>&lt;root&gt;/store/&lt;mod-guid&gt;/&lt;key&gt;</code> holds the only thing this project has that
...
are all in [`host/common/modstore.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modstore.nim):
...
[`tools/storecrash.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/storecrash.nim) is the evidence rather than the claim: it kills a writing
...
[`host/common/modcontrol.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modcontrol.nim) is the host half and is shared by both hosts,
because the wire is the same on either side and two implementations of it would
drift. [`mods/manager/mgr/control.nim`](https://github.com/aoughwl/aowlspt/blob/main/mods/manager/mgr/control.nim) is the mod half.
...
poll of <code>/aowlspt/mods/client/&lt;version&gt;</code> on the overlay's existing worker thread
...
log). <code>&lt;version&gt;</code> is the *client host's* own version, so the manager checks each
...
[`tools/livectl.nim`](https://github.com/aoughwl/aowlspt/blob/main/tools/livectl.nim) proves the server half every `aowl test` run: it disables a
...
[MODMANAGER.md](./modmanager) is the whole of it, including what is refused.
...
a firing reaches the original method. [`tests/detour_race.c`](https://github.com/aoughwl/aowlspt/blob/main/tests/detour_race.c) counted that: six
...
* **A firing never looks anything up.** There was a slot-indexed
  ...
  <code>mov r11, &lt;this hook's site&gt;</code> + a jump into one common thunk, where the site
...
3. Both flag and counter live outside any `seq` — fixed C arrays in
   [`host/common/modhost.nim`](https://github.com/aoughwl/aowlspt/blob/main/host/common/modhost.nim), `AOWL_MOD_SLOTS` (16384) of them, indexed by the
...
[MODMANAGER.md](./modmanager) quotes that one for `modEnter`/`modLeave`, where
...
[`backend/modrace.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/modrace.nim) is the gate, and it asserts on **answers**: sixteen
...
1. **[`abi/aowlspt_net.h`](https://github.com/aoughwl/aowlspt/blob/main/abi/aowlspt_net.h), poller thread.** `WSAPoll` reports the socket readable.
...
5. **Inflate.** `inflateBody` in [`backend/aowlbackend.nim`](https://github.com/aoughwl/aowlspt/blob/main/backend/aowlbackend.nim). An unframed body
...
2. **`hookReturnTyped("...::get_AimingSensitivity", onSens)`** in
   [`aowl/src/aowlspt/game.nim`](https://github.com/aoughwl/aowlspt/blob/main/aowl/src/aowlspt/game.nim) calls `patch_typed` across the ABI, having checked
...
target at any time. See [INSTALL.md](./install) and [`installer/README.md`](https://github.com/aoughwl/aowlspt/blob/main/installer/README.md).
...
| you want to | read |
|---|---|
| write a client mod | [MODDING.md](./modding) |
| write a server mod | [BACKEND.md](./backend) |
| understand the contract | [ABI.md](./abi) |
| know why there is no BepInEx | [IL2CPP.md](./il2cpp) |
| make a client mod fast | [PERF.md](./perf) |
| make the server fast | [PERF-SERVER.md](./perf-server) |
| find a bug | [DEBUGGING.md](./debugging) |
| know what the emulator answers | [EMULATOR.md](./emulator), [EMULATOR-COVERAGE.md](./emulator-coverage) |
| get real game data | [IMPORTDB.md](./importdb) |
| turn mods on and off live | [MODMANAGER.md](./modmanager) |
| install it | [INSTALL.md](./install) |
