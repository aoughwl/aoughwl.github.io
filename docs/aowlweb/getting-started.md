...
- **A built nimony checkout** sitting next to aowlweb — aowlweb consumes nimony's
...
The relative `--path` in `src/nim.cfg` assumes aowlweb and nimony are siblings:

```
git clone https://github.com/nim-lang/nimony
git clone https://github.com/aoughwl/aowlweb
```
...
### 3. Build the aowlweb plugins

From the aowlweb checkout:
...
for f in nc/*/*.c.nif; do
  ../aowlweb/bin/nim-js "$f" "${f%.c.nif}.js"
done
...
../aowlweb/bin/nim-wasm nc/hello/hello.c.nif hello.wasm --program
